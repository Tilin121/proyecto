    ...
    def test_arith_series_with_scalar(
        ...
        if all_arithmetic_operators == "__rmod__" and pa.types.is_binary(
            ...
            request.applymarker(
                pytest.mark.xfail(
                    raises=TypeError,
                    reason="Can only string multiply by an integer."))
...
def test_sort_values_dictionary():
    df = pd.DataFrame({"a": pd.Series(["x", "y"], dtype=ArrowDtype(
        pa.dictionary(pa.int32(), pa.string()))), "b": [1, 2], }, )
    expected = df.copy()
