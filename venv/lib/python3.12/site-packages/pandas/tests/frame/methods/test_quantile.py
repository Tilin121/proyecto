...
@pytest.fixture(params=[["linear", "single"],
                        ["nearest", "table"]], ids=lambda x: "-".join(x))
...
class TestDataFrameQuantile:
    @pytest.mark.parametrize("df,expected",
                             [[DataFrame({0: Series(pd.arrays.SparseArray([1,
                                                                           2])),
                                          1: Series(pd.arrays.SparseArray([3,
                                                                           4])),
                                          }),
                                 Series([1.5,
                                         3.5],
    ...
    def test_quantile(
        ...
        if interpolation == "linear":
            ...
            request.applymarker(
                pytest.mark.xfail(
                    using_array_manager,
                    reason="Name set incorrectly for arraymanager"))
    ...
    def test_quantile_nan(self, interp_method, request, using_array_manager):
        ...
        exp = Series([3.0, 2.5 if interpolation == "linear" else 3.0], index=[
            "a", "b"], name=0.5)
        tm.assert_series_equal(res, exp)
    ...
    def test_quantile_empty_no_rows_dt64(self, interp_method):
        ...
        exp = Series([pd.NaT, pd.NaT], index=["a", "b"],
                     dtype="datetime64[ns]", name=0.5)
        tm.assert_series_equal(res, exp)
...
class TestQuantileExtensionDtype:
    ...
    def test_quantile_ea(self, request, obj, index):
        ...
        expected = Series([index[4], index[0], index[-1]],
                          dtype=exp_dtype, index=qs, name="A")
    ...
    def test_quantile_ea_with_na(self, obj, index):
        ...
        # expected here assumes len(index) == 9
        expected = Series([index[4], index[1], index[-2]],
                          dtype=index.dtype, index=qs, name="A")
        expected = type(obj)(expected)
        tm.assert_equal(result, expected)
