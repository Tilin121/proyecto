...
class DataFrame(NDFrame, OpsMixin):
    ...
    def _values(
        ...
        if isinstance(mgr, ArrayManager):
            if len(
                    mgr.arrays) == 1 and not is_1d_only_ea_dtype(
                    mgr.arrays[0].dtype):
    ...
    @Substitution(header_type="bool or list of str",
    ...
    def dot(self, other: AnyArrayLike | DataFrame) -> DataFrame | Series:
        ...
        if isinstance(other, (Series, DataFrame)):
            ...
            if len(common) > len(
                    self.columns) or len(common) > len(
                    other.index):
    ...
    @deprecate_nonkeyword_arguments(version="3.0",
                                    allowed_args=["self",
                                                  "destination_table"],
                                    name="to_gbq")
    def to_gbq(
    ...
    def _sanitize_column(self,
                         value) -> tuple[ArrayLike,
                                         BlockValuesRefs | None]:
        """
        ...
        if is_list_like(periods):
            ...
            for period in periods:
                if not is_integer(period):
                    raise TypeError(
                        f"Periods must be integer, but {period} is {
                            type(period)}.")
                ...
                shifted_dataframes.append(
                    super() .shift(
                        ...
                        freq=freq,
                        axis=axis,
                        fill_value=fill_value) .add_suffix(
            ...
            if fill_value is lib.no_default:
                ...
                if periods > 0:
                    ...
                    for col in range(min(ncols, abs(periods))):
                        ...
                        result.insert(len(result.columns), label,
                                      filler, allow_duplicates=True)
    ...
    def set_index(
        ...
        *,
        ...
        if append:
            ...
            if isinstance(self.index, MultiIndex):
                arrays.extend(self.index._get_level_values(i)
                              for i in range(self.index.nlevels))
            else:
                arrays.append(self.index)
    ...
    def nlargest(self, n: int, columns: IndexLabel,
                 keep: NsmallestNlargestKeep = "first") -> DataFrame:
        """
    ...
    def _should_reindex_frame_op(
        ...
        if fill_value is None and level is None and axis == 1:
            ...
            if len(cols) and not (len(cols) == len(left_uniques)
                                  and len(cols) == len(right_uniques)):
    ...
    def _align_for_op(
        ...
        if isinstance(right, np.ndarray):
            ...
            if any(is_array_like(el) for el in right):
                raise ValueError(
                    f"Unable to coerce list of {
                        type(
                            right[0])} to Series/DataFrame")
            # GH#17901
            right = to_series(right)
