    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            ...
            try:
                ...
                raise IndexError(
                    f"index {key} is out of bounds for axis 0 with size {
                        len(self)}") from err
        elif is_scalar(key):
            raise IndexError(
    ...
    def take(  # type: ignore[override]
        ...
        **kwargs,
        ...
        if len(indices) == 0:
            ...
            if ind_max >= len(self):
                raise IndexError(
                    f"index {ind_max} is out of bounds for axis 0 with size {
                        len(self)}")
            ind_min = indices.min()
            if ind_min < -len(self):
                raise IndexError(
                    f"index {ind_min} is out of bounds for axis 0 with size {
                        len(self)}")
