...
class TestGetDummies:
    ...
    def test_dataframe_dummies_all_obj(self, df, sparse):
        ...
        expected = DataFrame({"A_a": [1, 0, 1], "A_b": [0, 1, 0], "B_b": [
            1, 1, 0], "B_c": [0, 0, 1]}, dtype=bool, )
        if sparse:
            expected = DataFrame(
                {
    ...
    def test_dataframe_dummies_drop_first_with_categorical(
        ...
        df["cat"] = Categorical(["x", "y", "y"])
        result = get_dummies(df, drop_first=True, sparse=sparse)
        expected = DataFrame({"C": [1, 2, 3], "A_b": [0, 1, 0], "B_c": [
            0, 0, 1], "cat_y": [0, 1, 1]})
        cols = ["A_b", "B_c", "cat_y"]
    ...
    def test_get_dummies_int_int(self):
        ...
        result = get_dummies(data)
        expected = DataFrame([[1, 0], [0, 1], [1, 0]],
                             columns=Categorical(["a", "b"]), dtype=bool)
        tm.assert_frame_equal(result, expected)
    ...
    def test_get_dummies_arrow_dtype(self):
        ...
        df = DataFrame({"name": Series(["a"], dtype=CategoricalDtype(
            Index(["a"], dtype=ArrowDtype(pa.string()))), ), "x": 1, })
        result = get_dummies(df)
        tm.assert_frame_equal(result, expected)
