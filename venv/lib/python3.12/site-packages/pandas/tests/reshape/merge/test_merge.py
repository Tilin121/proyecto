...
class TestMerge:
    ...
    def test_merge_non_string_columns(self):
        ...
        left = DataFrame({0: [1, 0, 1, 0], 1: [0, 1, 0, 0], 2: [
            0, 0, 2, 0], 3: [1, 0, 0, 3]})
    ...
    def test_intelligently_handle_join_key(self):
        ...
        left = DataFrame({"key": [1, 1, 2, 2, 3], "value": list(
            range(5))}, columns=["value", "key"])
        right = DataFrame(
    ...
    def test_merge_non_unique_indexes(self):
        ...
        df2 = DataFrame({"y": ["c", "d", "e", "f", "g", "h"]}, index=[
            dt3, dt3, dt2, dt2, dt, dt])
        _check_merge(df1, df2)
    ...
    def test_other_timedelta_unit(self, unit):
        ...
        exp = DataFrame({"entity_id": [101, 102], "days": np.array(
            ["nat", "nat"], dtype=dtype)}, columns=["entity_id", "days"], )
        tm.assert_frame_equal(result, exp)
    ...
    def test_merge_datetime64tz_values(self):
        ...
        expected = DataFrame({"key": [1,
                                      2,
                                      3],
                              "value_x": list(pd.date_range("20151010",
                                                            periods=2,
                                                            tz="US/Eastern")) + [pd.NaT],
                              "value_y": [pd.NaT] + list(pd.date_range("20151011",
                                                                       periods=2,
                                                                       tz="US/Eastern")),
                              })
        result = merge(left, right, on="key", how="outer")
    ...
    def test_merge_on_periods(self):
        left = DataFrame({"key": pd.period_range(
    ...
    def test_merge_period_values(self):
        left = DataFrame({"key": [1, 2], "value": pd.period_range(
            "20151010", periods=2, freq="D")})
        right = DataFrame({"key": [2, 3], "value": pd.period_range(
    ...
    def test_merge_readonly(self):
        ...
        data1 = DataFrame(np.arange(20).reshape((4, 5)) + 1,
                          columns=["a", "b", "c", "d", "e"])
        data2 = DataFrame(
...
@pytest.fixture
def right():
    return DataFrame({"X": Series(["foo", "bar"]).astype(
        CategoricalDtype(["foo", "bar"])), "Z": [1, 2], })
...
class TestMergeCategorical:
    ...
    def tests_merge_categorical_unordered_equal(self):
        # GH-19551
        df1 = DataFrame({"Foo": Categorical(["A", "B", "C"], categories=[
            "A", "B", "C"]), "Left": ["A0", "B0", "C0"], })

        df2 = DataFrame({"Foo": Categorical(["C", "B", "A"], categories=[
            "C", "B", "A"]), "Right": ["C1", "B1", "A1"], })
    ...
    def test_dtype_on_categorical_dates(self):
        ...
        df = DataFrame([[date(2001, 1, 1), 1.1], [date(
            2001, 1, 2), 1.3]], columns=["date", "num2"])
        df["date"] = df["date"].astype("category")

        df2 = DataFrame([[date(2001, 1, 1), 1.3], [date(
            2001, 1, 3), 1.4]], columns=["date", "num4"])
...
@pytest.mark.parametrize("index",
                         [Index([1,
                                 2],
                                dtype=dtyp,
                                name="index_col") for dtyp in tm.ALL_REAL_NUMPY_DTYPES] + [CategoricalIndex(["A",
                                                                                                             "B"],
                                                                                                            categories=["A",
                                                                                                                        "B"],
                                                                                                            name="index_col"),
...
def test_merge_series(on, left_on, right_on, left_index, right_index, nm):
    # GH 21220
    a = DataFrame({"A": [1, 2, 3, 4]}, index=MultiIndex.from_product(
        [["a", "b"], [0, 1]], names=["outer", "inner"]), )
    b = Series([1, 2, 3, 4], index=MultiIndex.from_product(
        [["a", "b"], [1, 2]], names=["outer", "inner"]), name=nm, )
    expected = DataFrame(
        {"A": [2, 4], "B": [1, 3]},
...
def test_merge_series_multilevel():
    ...
    a = DataFrame({"A": [1, 2, 3, 4]}, index=MultiIndex.from_product(
        [["a", "b"], [0, 1]], names=["outer", "inner"]), )
    b = Series([1, 2, 3, 4], index=MultiIndex.from_product(
        [["a", "b"], [1, 2]], names=["outer", "inner"]), name=("B", "C"), )
    with pytest.raises(
...
@pytest.mark.parametrize("col1, col2, suffixes, msg",
                         [("a",
                           ...
                           ("a",
                            "b",
                            "c"),
                             r"too many values to unpack \(expected 2\)"),
                             ("a",
                              "a",
                              tuple("a"),
...
def test_merge_equal_cat_dtypes(cat_dtype, reverse):
    ...
    df1 = DataFrame({"foo": Series(["a", "b", "c"]).astype(
        cat_dtypes["one"]), "left": [1, 2, 3]}).set_index("foo")
    ...
    df2 = DataFrame({"foo": Series(data_foo).astype(
        cat_dtypes[cat_dtype]), "right": data_right}).set_index("foo")
...
def test_merge_equal_cat_dtypes2():
    ...
    expected = DataFrame({"left": [1, 2], "right": [3, 2], "foo": Series(
        ["a", "b"]).astype(cat_dtype)}).set_index("foo")
