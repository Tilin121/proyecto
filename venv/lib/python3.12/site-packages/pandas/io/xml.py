    ...
    def _parse_nodes(self, elems: list[Any]) -> list[dict[str, str | None]]:
        ...
        if self.elems_only:
            ...
            dicts = [{**el.attrib,
                      ...
                      **{nm: ch.text if ch.text else None for nm,
                          ch in zip(self.names,
                                    el.findall("*"))},
                      } for el in elems]
        ...
        dicts = [{k.split("}")[1] if "}" in k else k: v for k,
                  v in d.items()} for d in dicts]
    ...
    def _parse_doc(
        ...
        with preprocess_data(handle_data) as xml_data:
            ...
            if isinstance(xml_data, io.StringIO):
                ...
                document = fromstring(
                    xml_data.getvalue().encode(
                        self.encoding), parser=curr_parser)
            else:
                document = parse(xml_data, parser=curr_parser)
