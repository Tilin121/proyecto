...
def test_apply_with_duplicated_non_sorted_axis(test_series):
    # GH 30667
    df = DataFrame([["x", "p"], ["x", "p"], ["x", "o"]],
                   columns=["X", "Y"], index=[1, 2, 2])
    if test_series:
...
def test_apply_numeric_coercion_when_datetime():
    ...
    # GH 15421
    df = DataFrame({"A": [10, 20, 30], "B": ["foo", "3", "4"], "T": [
        pd.Timestamp("12:31:22")] * 3})
...
def test_apply_aggregating_timedelta_and_datetime():
    ...
    msg = "DataFrameGroupBy.apply operated on the grouping columns"
    with tm.assert_produces_warning(DeprecationWarning, match=msg):
        result = df.groupby("clientid").apply(lambda ddf: Series(
...
def test_apply_multi_level_name(category):
    ...
    df = DataFrame({"A": np.arange(10), "B": b, "C": list(
        range(10)), "D": list(range(10))}).set_index(["A", "B"])
                         ...
                         [[False,
                           DataFrame([[1,
                             ...
                             [True,
                              Series([1,
                                      1],
                                     index=MultiIndex.from_tuples([(1,
                                                                    1),
                                                                   (2,
                                                                    2)],
                                                                  names=["a",
...
def test_builtins_apply(keys, f):
    ...
    assert_msg = f"invalid frame shape: {
        result.shape} (expected ({ngroups}, 3))"
