...
class TestSeriesDropDuplicates:
    @pytest.fixture(params=["int_", "uint", "float64",
                            "str_", "timedelta64[h]", "datetime64[D]"])
    ...
    def test_drop_duplicates_categorical_bool_na(self, nulls_fixture):
        ...
        expected = Series(Categorical([True, False, np.nan], categories=[
            True, False], ordered=True), index=[0, 1, 4], )
        tm.assert_series_equal(result, expected)
