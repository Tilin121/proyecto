    ...
    def _maybe_coerce_merge_keys(self) -> None:
        ...
        for lk, rk, name in zip(
            ...
            lk_is_object_or_string = is_object_dtype(
                lk.dtype) or is_string_dtype(lk.dtype)
            rk_is_object_or_string = is_object_dtype(
                rk.dtype) or is_string_dtype(rk.dtype)
        ...
        def _check_dtype_match(left: ArrayLike, right: ArrayLike, i: int):
            if left.dtype != right.dtype:
                if isinstance(left.dtype, CategoricalDtype) and isinstance(
                    ...
                    # for them here.
                    msg = (
                        f"incompatible merge keys [{i}] {
                            repr(
                                left.dtype)} and " f"{
                            repr(
                                right.dtype)}, both sides category, but not equal ones")
                else:
                    msg = (
                        f"incompatible merge keys [{i}] {
                            repr(
                                left.dtype)} and " f"{
                            repr(
                                right.dtype)}, must be the same type")
                raise MergeError(msg)
...
def _left_join_on_index(left_ax: Index,
                        right_ax: Index,
                        join_keys: list[ArrayLike],
                        sort: bool = False) -> tuple[Index,
                                                     npt.NDArray[np.intp] | None,
                                                     npt.NDArray[np.intp]]:
...
def _validate_operand(obj: DataFrame | Series) -> DataFrame:
    if isinstance(obj, ABCDataFrame):
        ...
        raise TypeError(
            f"Can only merge Series or DataFrame objects, a {
                type(obj)} was passed")
...
def _items_overlap_with_suffix(
    ...
    if not is_list_like(
            suffixes,
            allow_sets=False) or isinstance(
            suffixes,
            dict):
        raise TypeError(
