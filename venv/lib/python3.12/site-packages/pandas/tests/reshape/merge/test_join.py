...
class TestJoin:
    ...
    def test_join_float64_float32(self):
        ...
        s = DataFrame(np.random.default_rng(2).random(
            5).astype("float32"), columns=["md"])
    ...
    def test_join_many_non_unique_index(self):
        ...
        s = Series(
            np.repeat(
                np.arange(8), 2), index=np.repeat(
                np.arange(8), 2), name="TEST")
...
def _restrict_to_columns(group, columns, suffix):
    found = [
        c for c in group.columns if c in columns or c.replace(
            suffix, "") in columns]
...
def test_join_inner_multiindex_deterministic_order():
    ...
    expected = DataFrame({"e": [5], "f": [6]}, index=MultiIndex.from_tuples(
