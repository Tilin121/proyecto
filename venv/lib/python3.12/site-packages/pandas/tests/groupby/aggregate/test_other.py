...
def test_agg_timezone_round_trip():
    ...
    dates = [pd.Timestamp(
        f"2016-01-0{i:d} 12:00:00", tz="US/Pacific") for i in range(1, 5)]
...
def test_sum_uint64_overflow():
    ...
    index = Index([9223372036854775808, 9223372036854775810,
...
def test_agg_category_nansum(observed):
    ...
    df = DataFrame({"A": pd.Categorical(
        ["a", "a", "b"], categories=categories), "B": [1, 2, 3]})
    msg = "using SeriesGroupBy.sum"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = df.groupby("A", observed=observed).B.agg(np.nansum)
    expected = Series([3, 3, 0], index=pd.CategoricalIndex(
        ["a", "b", "c"], categories=categories, name="A"), name="B", )
    if observed:
...
def test_groupby_agg_err_catching(err_cls):
    ...
    data = make_data()[:5]
    df = DataFrame({"id1": [0, 0, 0, 1, 1], "id2": [
        0, 1, 0, 1, 1], "decimals": DecimalArray(data)})
