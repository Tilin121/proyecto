...
def single_level_multiindex():
    ...
    return MultiIndex(levels=[["foo", "bar", "baz", "qux"]], codes=[
        [0, 1, 2, 3]], names=["first"])
...
class TestMultiIndexLoc:
    ...
    def test_loc_no_second_level_index(self):
        ...
        res = df.loc[np.s_[:, "c", :]]
        expected = DataFrame(index=MultiIndex.from_product(
            [list("ab"), list("e")]), columns=["Val"])
        tm.assert_frame_equal(res, expected)
...
def test_loc_datetime_mask_slicing():
    ...
    df = DataFrame(data=[[1, 2], [3, 4], [5, 6], [7, 6]],
                   index=m_idx, columns=["C1", "C2"])
    result = df.loc[(
...
def test_loc_with_nan():
    ...
    result = df.loc[["a"]]
    expected = DataFrame({"col": [1]}, index=MultiIndex.from_tuples(
        [("a", 1)], names=["ind1", "ind2"]))
    tm.assert_frame_equal(result, expected)
...
def test_mi_columns_loc_list_label_order():
    ...
    result = df.loc[:, ["B", "A"]]
    expected = DataFrame(np.zeros((5, 4)), columns=MultiIndex.from_tuples(
        [("B", 1), ("B", 2), ("A", 1), ("A", 2)]), )
    tm.assert_frame_equal(result, expected)
...
def test_loc_get_scalar_casting_to_float():
    # GH#41369
    df = DataFrame({"a": 1.0, "b": 2}, index=MultiIndex.from_arrays(
        [[3], [4]], names=["c", "d"]))
    result = df.loc[(3, 4), "b"]
    assert result == 2
