...
def test_extract_expand_kwarg(any_string_dtype):
    ...
    expected = DataFrame([["BAD__", "BAD"], [np.nan, np.nan], [
...
def test_extract_expand_False_mixed_object():
    ser = Series(["aBAD_BAD", np.nan, "BAD_b_BAD", True,
...
def test_extract_expand_capture_groups(any_string_dtype):
    ...
    expected = DataFrame([[np.nan, np.nan], [np.nan, np.nan], [
        np.nan, np.nan]], dtype=any_string_dtype)
    tm.assert_frame_equal(result, expected)
...
def test_extractall_stringindex(any_string_dtype):
    ...
    expected = DataFrame({"digit": ["1", "2", "1"]}, index=MultiIndex.from_tuples(
        [(0, 0), (0, 1), (1, 0)], names=[None, "match"]), dtype=any_string_dtype, )
    tm.assert_frame_equal(result, expected)
