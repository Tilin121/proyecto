...
def test_transform_frame(on):
    ...
    df = DataFrame(np.random.default_rng(2).random(
        (10, 2)), columns=list("AB"), index=index)
...
@pytest.fixture
def df(index):
    frame = DataFrame(np.random.default_rng(2).random(
        (10, 2)), columns=list("AB"), index=index)
    return frame
...
@pytest.fixture(params=["df_resample",
                        "df_col_resample",
                        "df_mult_resample",
                        "df_grouper_resample"])
def cases(request):
...
def test_multi_agg_axis_1_raises(func):
    ...
    index.name = "date"
    df = DataFrame(np.random.default_rng(2).random(
        (10, 2)), columns=list("AB"), index=index).T
