...
class TestDataFrameSetItemWithExpansion:
    ...
    def test_setitem_with_expansion_categorical_dtype(self):
        # assignment
        df = DataFrame({"value": np.array(np.random.default_rng(
            2).integers(0, 10000, 100), dtype="int32")})
        labels = Categorical(
        ...
        expected = Series([np.dtype("int32"), CategoricalDtype(
            categories=labels, ordered=False)], index=["value", "D"], )
        tm.assert_series_equal(result, expected)
...
class TestDataFrameSetItemSlicing:
    ...
    def test_setitem_slice_broadcasting_rhs_mixed_dtypes(
            self, n, box, indexer):
        # GH#40440
        df = DataFrame([[1, 3, 5], ["x", "y", "z"]] +
                       [[2, 4, 6]] * n, columns=["a", "b", "c"])
        indexer(df)[1:] = box([10, 11, 12])
        expected = DataFrame(
            [[1, 3, 5]] + [[10, 11, 12]] * (n + 1),
            columns=["a", "b", "c"],
...
class TestDataFrameSetItemBooleanMask:
    ...
    def test_setitem_boolean_indexing(self):
        ...
        expected = DataFrame(index=idx, columns=cols, data=np.array(
...
class TestDataFrameSetitemCopyViewSemantics:
    ...
    def test_setitem_partial_column_inplace(
        ...
        df = DataFrame({"x": [1.1, 2.1, 3.1, 4.1], "y": [
            5.1, 6.1, 7.1, 8.1]}, index=[0, 1, 2, 3])
    ...
    def test_setitem_column_frame_as_category(self):
        ...
        expected_types = Series(["int64", "category", "category"], index=[
            0, "col1", "col2"], dtype=object)
