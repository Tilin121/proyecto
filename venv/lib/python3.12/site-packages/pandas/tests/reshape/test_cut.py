...
@pytest.mark.parametrize("x, bins, expected",
                         [(date_range("2017-12-31",
                                      periods=3),
                           [Timestamp.min,
                            Timestamp("2018-01-01"),
                             Timestamp.max],
                             IntervalIndex.from_tuples([(Timestamp.min,
                                                         Timestamp("2018-01-01")),
                                                        (Timestamp("2018-01-01"),
                                                         Timestamp.max),
                                                        ]),
                             ...
                             ([-1,
                              ...
                              np.array([np.iinfo(np.int64).min,
                                       ...
                                       dtype="int64"),
                              IntervalIndex.from_tuples([(np.iinfo(np.int64).min,
                                                         0),
                                                        (0,
                                                         np.iinfo(np.int64).max)]),
                              ),
                             ([np.timedelta64(-1,
                              ...
                              np.array([np.timedelta64(-np.iinfo(np.int64).max,
                                                       "ns"),
                                       np.timedelta64(0,
                                                      "ns"),
                              ...
                              IntervalIndex.from_tuples([(np.timedelta64(-np.iinfo(np.int64).max,
                                                                         "ns"),
                                                         np.timedelta64(0,
                                                                        "ns"),
                                                          ),
...
@pytest.mark.parametrize("get_labels,get_expected",
                         [(lambda labels: labels,
                           lambda labels: Categorical(["Medium"] + 4 * ["Small"] + ["Medium",
                                                                                    "Large"],
                                                      categories=labels,
                                                      ordered=True,
                                                      ),
                           ),
                          (lambda labels: Categorical.from_codes([0,
                                                                  1,
                                                                  2],
                                                                 labels),
                           lambda labels: Categorical.from_codes([1] + 4 * [0] + [1,
                                                                                  2],
                                                                 labels),
                           ),
                          ],
...
def test_datetime_nan_mask():
    result = cut(
        date_range(
            "20130102", periods=5), bins=date_range(
            "20130101", periods=2))
...
def test_cut_with_duplicated_index_lowest_included():
    # GH 42185
    expected = Series([Interval(-0.001,
                                2,
                                closed="right")] * 3 + [Interval(2,
                                                                 4,
                                                                 closed="right"),
                                                        Interval(-0.001,
                                                                 2,
...
def test_cut_with_nullable_int64():
    ...
    expected = Series(Categorical.from_codes(
