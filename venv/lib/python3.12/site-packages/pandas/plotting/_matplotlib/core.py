...
class MPLPlot(ABC):
    ...
    def _validate_subplots_kwarg(
        ...
        for group in subplots:
            ...
            if (idx_locs == -1).any():
                ...
                raise ValueError(
                    f"Column label(s) {
                        list(bad_labels)} not found in the DataFrame.")
    ...
    def _validate_color_args(self, color, colormap):
        ...
        if isinstance(
                color,
                tuple) and self.nseries == 1 and len(color) in (
                3,
                4):
    ...
    def _convert_to_ndarray(data):
        ...
        if (is_integer_dtype(data.dtype) or is_float_dtype(
                data.dtype)) and isinstance(data.dtype, ExtensionDtype):
