...
def test_ewma_with_times_variable_spacing(tz_aware_fixture, unit):
    tz = tz_aware_fixture
    halflife = "23 days"
    times = (DatetimeIndex(["2020-01-01", "2020-01-10T00:04:05",
                            "2020-02-23T05:00:23"]) .tz_localize(tz) .as_unit(unit))
    data = np.arange(3)
    df = DataFrame(data)
...
def test_float_dtype_ewma(func, expected, float_numpy_dtype):
    ...
    df = DataFrame({0: range(5), 1: range(6, 11), 2: range(
        10, 20, 2)}, dtype=float_numpy_dtype)
...
def test_times_string_col_raises():
    # GH 43265
    df = DataFrame({"A": np.arange(10.0), "time_col": date_range(
...
@pytest.mark.parametrize("s, adjust, ignore_na, w",
                         [(Series([np.nan,
                                   1.0,
                                   101.0]),
                           True,
