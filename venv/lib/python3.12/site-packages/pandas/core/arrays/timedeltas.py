...
class TimedeltaArray(dtl.TimelikeOps):
    ...
    def sum(
        ...
        *,
        ...
        nv.validate_sum((), {"dtype": dtype, "out": out,
    ...
    def _add_offset(self, other):
        ...
        raise TypeError(
            f"cannot add the type {
                type(other).__name__} to a {
                type(self).__name__}")
    ...
    def _vector_divlike_op(self, other, op) -> np.ndarray | Self:
        ...
        if (is_integer_dtype(other.dtype) or is_float_dtype(other.dtype)
            ) and op in [operator.truediv, operator.floordiv, ]:
            return type(self)._simple_new(result, dtype=result.dtype)
...
def sequence_to_td64ns(
    ...
    if data.dtype == object or is_string_dtype(data.dtype):
        ...
        raise TypeError(
            f"dtype {
                data.dtype} cannot be converted to timedelta64[ns]")
