...
def ensure_python_int(value: int | np.integer) -> int:
    ...
    if not (is_integer(value) or is_float(value)):
        if not is_scalar(value):
            raise TypeError(
                f"Value needs to be a scalar value, was type {
                    type(value).__name__}")
...
def is_integer_dtype(arr_or_dtype) -> bool:
    ...
    return _is_dtype_type(
        arr_or_dtype, _classes_and_not_datetimelike(
            np.integer)) or _is_dtype(
...
def is_unsigned_integer_dtype(arr_or_dtype) -> bool:
    ...
    return _is_dtype_type(
        arr_or_dtype, _classes_and_not_datetimelike(
            np.unsignedinteger)) or _is_dtype(
        arr_or_dtype, lambda typ: isinstance(
...
def is_numeric_dtype(arr_or_dtype) -> bool:
    ...
    return _is_dtype_type(
        arr_or_dtype, _classes_and_not_datetimelike(
            np.number, np.bool_)) or _is_dtype(
        arr_or_dtype, lambda typ: isinstance(
...
def is_float_dtype(arr_or_dtype) -> bool:
    ...
    """
    return _is_dtype_type(
        arr_or_dtype, classes(
            np.floating)) or _is_dtype(
        arr_or_dtype, lambda typ: isinstance(
