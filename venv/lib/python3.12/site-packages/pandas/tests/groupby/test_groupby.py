...
def test_with_na_groups(dtype):
    ...
    labels = Series([np.nan, "foo", "bar", "bar", np.nan,
                     np.nan, "bar", "bar", np.nan, "foo"], index=index, )
...
def test_multi_func(df):
    ...
    tm.assert_frame_equal(agged.loc[:, ["C", "D"]], expected.loc[:, [
...
def test_frame_multi_key_function_list():
    ...
    expected = pd.concat([grouped["D"].agg(funcs), grouped["E"].agg(
        funcs), grouped["F"].agg(funcs)], keys=["D", "E", "F"], axis=1, )
...
def test_as_index_select_column():
    ...
    expected = Series([2, 6, 6], name="B", index=MultiIndex.from_tuples(
        [(0, 0), (0, 1), (1, 2)]))
    tm.assert_series_equal(result, expected)
...
def test_groupby_with_hier_columns():
    ...
    df = DataFrame(
        np.random.default_rng(2).standard_normal(
            (8, 4)), index=index, columns=columns)
...
def test_dont_clobber_name_column():
    df = DataFrame({"key": ["a", "a", "a", "b", "b", "b"], "name": [
        "foo", "bar", "baz"] * 2})
...
def test_groupby_multiindex_not_lexsorted():
    ...
    not_lexsorted_df = DataFrame(columns=["a", "b", "c", "d"], data=[
        [1, "b1", "c1", 3], [1, "b2", "c2", 4]])
    ...
    for level in [0, 1, [0, 1]]:
        for sort in [False, True]:
            result = df.groupby(
                level=level, sort=sort, group_keys=False).apply(
...
@pytest.mark.parametrize("group_column",
                         ["int_groups",
                          "string_groups",
                          ["int_groups",
                           "string_groups"]])
...
def test_pivot_table_values_key_error():
    ...
    df = DataFrame({"eventDate": date_range(datetime.today(),
                   periods=20, freq="ME").tolist(), "thename": range(20), })
...
def test_groupby_duplicate_columns(infer_string):
    ...
    expected = DataFrame([["e", "a", 1]], index=np.array(
        [0]), columns=["A", "B", "B"], dtype=object)
    tm.assert_frame_equal(result, expected)
...
def test_groupby_numerical_stability_cumsum():
    ...
    exp_data = ([1e16] * 2 + [1e16 + 96, 1e16 + 98] +
...
@pytest.mark.parametrize("level_arg, multiindex",
