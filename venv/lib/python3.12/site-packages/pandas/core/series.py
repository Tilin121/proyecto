...
class Series(base.IndexOpsMixin, NDFrame):  # type: ignore[misc]
    ...
    _hidden_attrs = (base.IndexOpsMixin._hidden_attrs |
                     NDFrame._hidden_attrs | frozenset([]))
    ...
    def __init__(
        ...
        if isinstance(
                data,
                SingleBlockManager) and using_copy_on_write() and not copy:
            data = data.copy(deep=False)
    ...
    def ravel(self, order: str = "C") -> ArrayLike:
        ...
        warnings.warn(
            ...
            "array instead.", FutureWarning, stacklevel=2, )
    ...
    def argsort(
        ...
        if mask.any():
            ...
            warnings.warn(
                ...
                "last instead of set to -1.", FutureWarning, stacklevel=find_stack_level(), )
    ...
    def case_when(self,
                  caselist: list[tuple[ArrayLike | Callable[[Series],
                                                            Series | np.ndarray | Sequence[bool]],
                                       ArrayLike | Scalar | Callable[[Series],
                                                                     Series | np.ndarray],
                                       ],
                                 ],
                  ) -> Series:
        ...
        if not isinstance(caselist, list):
            raise TypeError(
                f"The caselist argument should be a list; instead got {
                    type(caselist)}")
        ...
        for num, entry in enumerate(caselist):
            if not isinstance(entry, tuple):
                raise TypeError(
                    f"Argument {num} must be a tuple; instead got {
                        type(entry)}.")
            if len(entry) != 2:
                raise ValueError(
        ...
        for position, condition, replacement in zip(
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to apply condition{position} and replacement{position}.") from error
        return default
            ...
            if not left.index.equals(right.index):
                if align_asobject:
                    if left.dtype not in (
                            object, np.bool_) or right.dtype not in (
                            object, np.bool_, ):
                        warnings.warn(
