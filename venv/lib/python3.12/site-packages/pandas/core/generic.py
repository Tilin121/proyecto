...
class NDFrame(PandasObject, indexing.IndexingMixin):
    ...
    def _get_axis_number(cls, axis: Axis) -> AxisInt:
        try:
            ...
            raise ValueError(
                f"No axis named {axis} for object type {
                    cls.__name__}")
    ...
    def _get_index_resolvers(self) -> dict[Hashable, Series | MultiIndex]:
        ...
        return {
            clean_column_name(k): v for k,
            v in d.items() if not isinstance(
                k,
                int)}
    ...
    def abs(self) -> Self:
        ...
        return self._constructor_from_mgr(
            res_mgr, axes=res_mgr.axes).__finalize__(
            self, name="abs")
    ...
    def _is_label_or_level_reference(
        ...
        return self._is_level_reference(
            key, axis=axis) or self._is_label_reference(
            key, axis=axis)
    ...
    def _drop_labels_or_levels(self, keys, axis: AxisInt = 0):
        ...
        invalid_keys = [
            k for k in keys if not self._is_label_or_level_reference(
                k, axis=axis)]
    ...
    def __array__(
        ...
        if (
            ...
            if astype_is_view(
                    self.dtypes.iloc[0],
                    values.dtype) and astype_is_view(
                    values.dtype,
                    arr.dtype):
                arr = arr.view()
    ...
    def take(self, indices, axis: Axis = 0, **kwargs) -> Self:
        ...
        **kwargs
        ...
        return self._constructor_from_mgr(
            new_data, axes=new_data.axes).__finalize__(
            self, method="take")
    ...
    def _reindex_with_indexers(
        ...
        return self._constructor_from_mgr(
            new_data, axes=new_data.axes).__finalize__(self)
    ...
    def _consolidate(self):
        ...
        return self._constructor_from_mgr(
            cons_data, axes=cons_data.axes).__finalize__(self)
        ...
        if value is lib.no_default or method is not lib.no_default:
            ...
            if is_dict_like(to_replace):
                ...
                # {'A': NA} -> 0
                    ...
                    mapping = {col: (to_rep, value)
                               for col, to_rep in to_replace.items()}
                    return self._replace_columnwise(mapping, inplace, regex)
                else:
                    raise TypeError(
    ...
    def compare(
        ...
        indices = (np.arange(diff.shape[axis]).reshape(
    ...
    def shift(
        ...
        if freq is not None and fill_value is not lib.no_default:
            ...
            warnings.warn(
                ...
                "version.", FutureWarning, stacklevel=find_stack_level(), )
    ...
    def _accum_func(
        ...
        **kwargs,
        ...
        return self._constructor_from_mgr(
            result, axes=result.axes).__finalize__(
            self, method=name)
    ...
    def __iadd__(self, other) -> Self:
        ...
        return self._inplace_method(
            other, type(self).__add__)  # type: ignore[operator]

    @final
    def __isub__(self, other) -> Self:
        # error: Unsupported left operand type for - ("Type[NDFrame]")
        return self._inplace_method(
            other, type(self).__sub__)  # type: ignore[operator]

    @final
    def __imul__(self, other) -> Self:
        # error: Unsupported left operand type for * ("Type[NDFrame]")
        return self._inplace_method(
            other, type(self).__mul__)  # type: ignore[operator]
    ...
    def __imod__(self, other) -> Self:
        # error: Unsupported left operand type for % ("Type[NDFrame]")
        return self._inplace_method(
            other, type(self).__mod__)  # type: ignore[operator]

    @final
    def __ipow__(self, other) -> Self:
        # error: Unsupported left operand type for ** ("Type[NDFrame]")
        return self._inplace_method(
            other, type(self).__pow__)  # type: ignore[operator]

    @final
    def __iand__(self, other) -> Self:
        # error: Unsupported left operand type for & ("Type[NDFrame]")
        return self._inplace_method(
            other, type(self).__and__)  # type: ignore[operator]
    ...
    def __ixor__(self, other) -> Self:
        # error: Unsupported left operand type for ^ ("Type[NDFrame]")
        return self._inplace_method(
            other, type(self).__xor__)  # type: ignore[operator]
