...
"""
    if "dayfirst" in kwargs:
        ...
        exp_index = Index([datetime(2010, 1, 31), datetime(
            2010, 2, 1), datetime(2010, 2, 2)], name="time", )
        expected = DataFrame(
...
def test_parse_tz_aware(all_parsers):
    ...
    expected = DataFrame({"x": [0.5]}, index=Index(
        [Timestamp("2012-06-13 01:39:00+00:00")], name="Date"))
    if parser.engine == "pyarrow":
...
"""
    ...
    with tm.assert_produces_warning(
        ...
        result = parser.read_csv(
            StringIO(data), index_col=[
                "nominal", "ID"], parse_dates={
                "nominal": [
                    1, 2]})
    with tm.assert_produces_warning(
        (FutureWarning, DeprecationWarning), match=depr_msg, check_stacklevel=False
...
2020-12-31,31-12-2020"""
    ...
    result = parser.read_csv(
        ...
        parse_dates=True,
        index_col=0)
    expected = DataFrame({"b": ["31-12-2019", "31-12-2020"], }, index=Index(
