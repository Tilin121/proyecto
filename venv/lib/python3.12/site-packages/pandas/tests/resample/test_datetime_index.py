...
def test_custom_grouper_df(index, unit):
    ...
    df = DataFrame(np.random.default_rng(2).random(
        (len(dti), 10)), index=dti, dtype="float64")
...
def test_resample_basic_from_daily(unit):
    ...
    dti = date_range(
        start=datetime(
            ...
            2005, 1, 10), freq="D", name="index").as_unit(unit)
...
def test_nearest_upsample_with_limit(tz_aware_fixture, freq, rule, unit):
    ...
    rng = date_range("1/1/2000", periods=3, freq=freq,
                     tz=tz_aware_fixture).as_unit(unit)
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), rng)
...
def test_resample_single_group_std(unit):
    ...
    expected = Series([0.75], index=DatetimeIndex(
        [Timestamp("20070915")], freq="D").as_unit(unit))
...
def test_resample_size(unit):
    ...
    ix = date_range(
        start=left.index.min(),
        end=ts.index.max(),
        freq="7min").as_unit(unit)
...
def test_resample_with_nat(unit):
    ...
    index_2s = DatetimeIndex(
        ["1970-01-01 00:00:00", "1970-01-01 00:00:02"]).as_unit(unit)
