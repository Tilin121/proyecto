...
def obtener_partidos_proximos(liga_nombre=None, dias=7):
    ...
    if liga_nombre:
        cursor.execute(
            "SELECT id, nombre, url FROM ligas WHERE nombre = %s", (liga_nombre,)
        )
    else:
    ...
    for liga_id, liga_nombre, liga_url in ligas:
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"❌ Error al obtener calendario de {liga_nombre}. Código {
            ...
            for elemento in contenedor_partidos.find_all(["h3", "div", "table"]):
                ...
                elif fecha_actual and (
                    elemento.name == "div" or elemento.name == "table"
                ):
                    filas_partido = (
                        elemento.find_all("tr")
                        if elemento.name == "table"
                        else elemento.find_all("div", class_="match")
                    )

                    for fila in filas_partido:
                        try:
                            # Extraer equipos
                            equipos = fila.find_all(
                                "td", class_="team-name"
                            ) or fila.find_all("div", class_="team")
                            if len(equipos) < 2:
                                continue
                            ...
                            # Extraer hora
                            hora_elem = fila.find("td", class_="time") or fila.find(
                                "div", class_="time"
                            )
                            ...
                            fecha_hora = f"{
                                fecha_actual.strftime('%Y-%m-%d')} {hora}"
                            ...
                            if (
                                ...
                                elementos_cuota = fila.find_all(
                                    "td", class_="odds"
                                ) or fila.find_all("div", class_="odds")
            ...
            for partido in partidos:
                try:
                    ...
                    cursor.execute(
                        "SELECT id FROM equipos WHERE nombre = %s",
                    ...
                    print(
                        f"❌ Error al guardar partido {
                            partido['equipo_local']} vs {
                            partido['equipo_visitante']}: {e}"
                    )
    ...
    print(
        f"\n✅ Total: {
            len(partidos_totales)} partidos próximos extraídos y guardados."
    )
...
def extraer_estadisticas_historicas(equipo_id=None, max_partidos=20):
    ...
    for equipo_id, equipo_nombre in equipos:
        ...
        url_busqueda = f"https://fbref.com/en/search/search.fcgi?search={
            equipo_nombre.replace(
                ' ', '+')}"
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"❌ Error al buscar {equipo_nombre}. Código {
                        response.status_code}"
                )
                continue
            ...
            if response.status_code != 200:
                print(
                    f"❌ Error al acceder a página de {equipo_nombre}. Código {
                        response.status_code}"
                )
                continue
            ...
            for fila in filas[:max_partidos]:  # Limitar a max_partidos
                try:
                    ...
                    goles_favor = int(
                        fila.find("td", {"data-stat": "goals_for"}).text.strip() or 0
                    )
                    goles_contra = int(
                        fila.find("td", {"data-stat": "goals_against"}).text.strip()
                        or 0
                    )
                    ...
                    posesion = (
                        int(posesion.text.strip().replace("%", ""))
                        if posesion
                        else None
                    )
...
def obtener_datos_cuotas_casas_apuestas(dias=7, casas_apuestas=None):
    ...
    try:
        ...
        for partido_id, equipo_local, equipo_visitante, liga, fecha in partidos:
            partido_key = f"{partido_id}_{
                equipo_local.replace(
                    ' ', '_')}_{
                equipo_visitante.replace(
                    ' ', '_')}"
        ...
        conn.commit()
        print(
            f"✅ Cuotas extraídas y guardadas para {
                len(cuotas_resultado)} partidos."
        )
...
def extraer_cuotas_bet365(cuotas_resultado):
    ...
    try:
        ...
        for partido_key, partido_info in cuotas_resultado.items():
            ...
            if response.status_code != 200:
                print(
                    f"⚠️ Error al acceder a Bet365: Código {
                        response.status_code}"
                )
                continue
            ...
            # Simular cuotas extraídas (en producción, esto vendría del
            # scraping real)
            cuotas_simuladas = {
            ...
            print(
                f"✅ Cuotas obtenidas para {equipo_local} vs {equipo_visitante} en Bet365"
            )
...
def extraer_cuotas_betfair(cuotas_resultado):
    ...
    try:
        ...
        for partido_key, partido_info in cuotas_resultado.items():
            ...
            print(
                f"✅ Cuotas obtenidas para {
                    partido_info['local']} vs {
                    partido_info['visitante']} en Betfair"
            )
            time.sleep(random.uniform(1, 2))
    ...
    try:
        ...
        for partido_key, partido_info in cuotas_resultado.items():
            ...
            print(
                f"✅ Cuotas obtenidas para {
                    partido_info['local']} vs {
                    partido_info['visitante']} en William Hill"
            )
            time.sleep(random.uniform(1, 2))
    ...
    try:
        ...
        for partido_key, partido_info in cuotas_resultado.items():
            ...
            print(
                f"✅ Cuotas obtenidas para {
                    partido_info['local']} vs {
                    partido_info['visitante']} en Bwin"
            )
            time.sleep(random.uniform(1, 2))
