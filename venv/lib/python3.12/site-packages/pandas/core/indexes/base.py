...
class Index(IndexOpsMixin, PandasObject):
    ...
    _data_cls: type[ExtensionArray] | tuple[type[np.ndarray],
                                            type[ExtensionArray]] = (np.ndarray, ExtensionArray, )
    ...
    def __array_wrap__(self, result, context=None, return_scalar=False):
        ...
        """
        result = lib.item_from_zerodim(result)
        if (not isinstance(result, Index) and is_bool_dtype(
                result.dtype)) or np.ndim(result) > 1:
        ...
        if cls is not None and not hasattr(cls, "_typ"):
            ...
            if cls is not None:
                warnings.warn(
                    # GH#55709
                    f"Passing a type in {
                        type(self).__name__}.view is deprecated "
    ...
    def _get_level_names(self) -> Hashable | Sequence[Hashable]:
        ...
        if self._is_multi:
            return [
                level if name is None else name for level,
                name in enumerate(
                    self.names)]
        else:
    ...
    def _set_names(self, values, *, level=None) -> None:
        ...
        if len(values) != 1:
            raise ValueError(
                f"Length of new names must be 1, got {
                    len(values)}")
    ...
    def _validate_index_level(self, level) -> None:
        ...
        if isinstance(level, int):
            ...
            raise KeyError(
                f"Requested level ({level}) does not match index name ({
                    self.name})")
    ...
    def fillna(self, value=None, downcast=lib.no_default):
        ...
        if not is_scalar(value):
            raise TypeError(
                f"'value' must be a scalar, passed: {
                    type(value).__name__}")
    ...
    def _difference(self, other, sort):
        ...
        if isinstance(
                self,
                ABCCategoricalIndex) and self.hasnans and other.hasnans:
            this = this.dropna()
        other = other.unique()
    ...
    def _check_indexing_method(
        ...
        if isinstance(self.dtype, (IntervalDtype, CategoricalDtype)):
            ...
            if method is not None:
                raise NotImplementedError(
                    f"method {method} not yet implemented for {
                        type(self).__name__}")
    ...
    def _convert_tolerance(
        ...
        if target.size != tolerance.size and tolerance.size > 1:
            ...
            if tolerance.ndim > 0:
                raise ValueError(
                    f"tolerance argument for {
                        type(self).__name__} with dtype " f"{
                        self.dtype} must contain numeric elements if it is list type")

            raise ValueError(
                f"tolerance argument for {
                    type(self).__name__} with dtype {
                    self.dtype} " f"must be numeric if it is a scalar: {
                    repr(tolerance)}")
        return tolerance
    ...
    def join(self,
        ...
        if isinstance(
                self,
                ABCDatetimeIndex) and isinstance(
                other,
                ABCDatetimeIndex):
    ...
    @final
    def _join_empty(self,
                    other: Index,
                    how: JoinHow,
                    sort: bool) -> tuple[Index,
                                         npt.NDArray[np.intp] | None,
                                         npt.NDArray[np.intp] | None]:
    ...
    @final
    def _join_level(self,
                    other: Index,
                    level,
                    how: JoinHow = "left",
                    keep_order: bool = True) -> tuple[MultiIndex,
                                                      npt.NDArray[np.intp] | None,
                                                      npt.NDArray[np.intp] | None]:
        """
    ...
    def _can_use_libjoin(self) -> bool:
        ...
        if type(self) is Index:
            ...
            return (
                isinstance(
                    self.dtype,
                    np.dtype) or isinstance(
                    self._values,
                    (ArrowExtensionArray,
                     BaseMaskedArray)) or self.dtype == "string[python]")
            ...
            if not isinstance(self.dtype, ExtensionDtype):
                if len(key) == 0 and len(key) != len(self):
                    warnings.warn(
                        ...
                        "future version.", FutureWarning, stacklevel=find_stack_level(), )
    ...
    def _get_indexer_non_comparable(
        ...
        if method is not None:
            ...
            raise TypeError(
                f"Cannot compare dtypes {
                    self.dtype} and {other_dtype}")
    ...
    def slice_locs(self, start=None, end=None, step=None) -> tuple[int, int]:
        ...
        if isinstance(
                start, (str, datetime)) and isinstance(
                end, (str, datetime)):
            try:
...
def get_values_for_csv(
    ...
    *,
    ...
    if isinstance(
            values,
            Categorical) and values.categories.dtype.kind in "Mm":
