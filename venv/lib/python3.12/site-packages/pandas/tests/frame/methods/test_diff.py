...
class TestDataFrameDiff:
    ...
    def test_diff_datetime_axis0_with_nat(self, tz, unit):
        ...
        result = df.diff()
        ex_index = pd.TimedeltaIndex(
            [pd.NaT, pd.NaT, pd.Timedelta(days=1)]).as_unit(unit)
    ...
    def test_diff_timedelta(self, unit):
        # GH#4533
        df = DataFrame({"time": [Timestamp("20130101 9:01"), Timestamp(
            "20130101 9:02")], "value": [1.0, 2.0], })
        df["time"] = df["time"].dt.as_unit(unit)

        res = df.diff()
        exp = DataFrame([[pd.NaT, np.nan], [pd.Timedelta(
            "00:01:00"), 1]], columns=["time", "value"])
    ...
    def test_diff_axis1_mixed_dtypes(self):
        ...
        df = DataFrame({"a": np.arange(3, dtype="float32"),
