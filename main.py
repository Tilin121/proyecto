...
def mostrar_prediccion(prediccion):
    ...
    print("\n📊 PREDICCIÓN:")
    print(
        f"▶️ Resultado más probable: {
            pred['resultado_nombre']} ({
            formatear_porcentaje(
                pred['probabilidades'][
                    pred['resultado_nombre'].lower().replace(
                        'victoria ',
                        '')])})"
    )
    ...
    probabilidades = [
        [
            "Victoria Local",
            formatear_porcentaje(pred["probabilidades"]["local"]),
            formatear_decimal(partido["cuotas"]["Local"]),
            formatear_decimal(pred["valor_apuestas"]["local"]),
        ],
        [
            "Empate",
            formatear_porcentaje(pred["probabilidades"]["empate"]),
            formatear_decimal(partido["cuotas"]["Empate"]),
            formatear_decimal(pred["valor_apuestas"]["empate"]),
        ],
        [
            "Victoria Visitante",
            formatear_porcentaje(pred["probabilidades"]["visitante"]),
            formatear_decimal(partido["cuotas"]["Visitante"]),
            formatear_decimal(pred["valor_apuestas"]["visitante"]),
        ],
    ]
    print(
        tabulate(
            probabilidades,
            headers=["Resultado", "Probabilidad", "Cuota", "Valor"],
            tablefmt="grid",
        )
    )
    ...
    for pred in predicciones:
        ...
        filas.append(
            [
                ...
                formatear_decimal(
                    partido["cuotas"][p["apuesta_recomendada"].replace("Victoria ", "")]
                ),
                formatear_porcentaje(
                    p["probabilidades"][
                        p["apuesta_recomendada"].lower().replace("victoria ", "")
                    ]
                ),
...
def mostrar_estadisticas_modelo(stats):
    ...
    print(
        f"▶️ Retorno de inversión (ROI): {
            formatear_porcentaje(
                stats['retorno_inversion'] /
                100)}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Sistema de Predicción de Apuestas Deportivas"
    )
