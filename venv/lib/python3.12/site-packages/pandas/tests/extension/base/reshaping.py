...
class BaseReshapingTests:
    ...
    def test_concat_columns(self, data, na_value):
        ...
        expected = pd.DataFrame({"A": data._from_sequence(
            list(data[:3]) + [na_value], dtype=data.dtype), "B": [np.nan, 1, 2, 3], })
    ...
    def test_concat_extension_arrays_copy_false(self, data, na_value):
        ...
        expected = pd.DataFrame({"A": data._from_sequence(
            list(data[:3]) + [na_value], dtype=data.dtype), "B": data[3:7], })
    ...
    def test_unstack(self, data, index, obj):
        ...
        for level in combinations:
            ...
            assert all(
                isinstance(
                    result[col].array,
                    type(data)) for col in result.columns)
    ...
    def test_ravel(self, data):
        ...
        if data.dtype._is_immutable:
            pytest.skip(
                f"test_ravel assumes mutability and {
                    data.dtype} is immutable")
    ...
    def test_transpose(self, data):
        ...
        if data.dtype._is_immutable:
            pytest.skip(
                f"test_transpose assumes mutability and {
                    data.dtype} is immutable")
