...
def obtener_estadisticas():
    ...
    if response.status_code != 200:
        print(f"❌ Error al obtener datos de FBRef. Código {response.status_code}")
        return
    ...
    if jugador:
        ...
        cursor.execute("INSERT INTO jugadores (nombre) VALUES (%s) RETURNING id", (nombre_jugador,))
        jugador = cursor.fetchone()
        if jugador:
            jugador_id = jugador[0]
        else:
    ...
    if stats:
        ...
        if ultima_fila:
            ...
            asistencias = obtener_valor(ultima_fila.find("td", {"data-stat": "assists"}))
            tiros = obtener_valor(ultima_fila.find("td", {"data-stat": "shots_total"}))
            tiros_a_puerta = obtener_valor(ultima_fila.find("td", {"data-stat": "shots_on_target"}))
            tarjetas_amarillas = obtener_valor(ultima_fila.find("td", {"data-stat": "cards_yellow"}))
            tarjetas_rojas = obtener_valor(ultima_fila.find("td", {"data-stat": "cards_red"}))
            ...
            """, (jugador_id, minutos, goles, asistencias, tiros, tiros_a_puerta, tarjetas_amarillas, tarjetas_rojas))
