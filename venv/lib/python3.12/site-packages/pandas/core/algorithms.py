...
def mode(values: ArrayLike, dropna: bool = True,
         mask: npt.NDArray[np.bool_] | None = None) -> ArrayLike:
    """
...
def searchsorted(
    ...
    return arr.searchsorted(
        value,
        side=side,
        sorter=sorter)  # type: ignore[arg-type]
    ...
    if not isinstance(arr, np.ndarray):
        ...
        if hasattr(arr, f"__{op.__name__}__"):
            if axis != 0:
                raise ValueError(
                    f"cannot diff {
                        type(arr).__name__} on axis={axis}")
