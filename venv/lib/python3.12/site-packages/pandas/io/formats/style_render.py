        ...
        if clabels:
            ...
            for c, value in enumerate(clabels[last_level]):
                ...
                if header_element_visible:
                    visible_col_count += 1
                if self._check_trim(
                    visible_col_count, max_cols, column_blanks, "th", f"{
                        self.css['blank']} {
                        self.css['col']}{c} {
                        self.css['col_trim']}", self.css["blank_value"], ):
                    break
    ...
    def _translate_body(self, idx_lengths: dict, max_rows: int, max_cols: int):
        ...
        for r, row_tup in [
            z for z in enumerate(
                self.data.itertuples()) if z[0] not in self.hidden_rows]:
            visible_row_count += 1
            if self._check_trim(
                visible_row_count,
                max_rows,
    ...
    def _generate_trimmed_row(self, max_cols: int) -> list:
        ...
        for c, _ in enumerate(self.columns):
            ...
            if data_element_visible:
                visible_col_count += 1
            if self._check_trim(
                visible_col_count, max_cols, data, "td", f"{
                    self.css['data']} {
                    self.css['row_trim']} {
                    self.css['col_trim']}", ):
                break
    ...
    "cellstyle": self.ctx_columns[r,
     c - visible_index_level_n]}
        ...
        if clines is not None:
            ...
            visible_row_indexes: list[int] = [r for r in range(
                len(self.data.index)) if r not in self.hidden_rows]
            ...
            for rn, r in enumerate(visible_row_indexes):
                for lvln, lvl in enumerate(visible_index_levels):
                    ...
                    if idx_len is not None:  # i.e. not a sparsified entry
                        d["clines"][rn +
    idx_len].append(f"\\cline{{lvln +
                                1} -
                               {len(visible_index_levels) +
                                 data_len}} ")
...
def _maybe_wrap_formatter(
    ...
    if isinstance(formatter, str):
        ...
        raise TypeError(
            f"'formatter' expected str or callable, got {
                type(formatter)}")
