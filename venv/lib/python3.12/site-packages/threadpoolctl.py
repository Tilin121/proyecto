...
class LibController(ABC):
    ...
    def _get_symbol(self, name):
        ...
        return getattr(
            self.dynlib, f"{
                self._symbol_prefix}{name}{
                self._symbol_suffix}", None)
...
class _ThreadpoolLimiter:
    ...
    def _check_params(self, limits, user_api):
        ...
        if isinstance(
                limits,
                str) and limits == "sequential_blas_under_openmp":
            (
                limits,
                user_api,
        ...
        try:
            ...
            for h_module in h_modules:
                ...
                if len(filepath) == max_path:  # pragma: no cover
                    ...
                    # Store the library controller if it is supported and
                    # selected
                    self._make_controller_from_path(filepath)
        finally:
