    ...
    def _validate(self) -> None:
        ...
        if self.min_periods is not None:
            ...
            if is_integer(self.window) and self.min_periods > self.window:
                raise ValueError(
                    f"min_periods {
                        self.min_periods} must be <= window {
                        self.window}")
    ...
    def _check_window_bounds(
        ...
        if len(start) != len(end):
            raise ValueError(
                f"start ({
                    len(start)}) and end ({
                    len(end)}) bounds must be the " f"same length")
...
class BaseWindowGroupby(BaseWindow):
    ...
    def _apply_pairwise(
        ...
        if other is not None and not all(len(group) == len(
                other) for group in self._grouper.indices.values()):
            # GH 42915
            ...
            gb_pairs = (com.maybe_make_list(pair)
                        for pair in self._grouper.indices.keys())
            groupby_codes = []
            groupby_levels = []
    ...
    def _create_data(
        ...
        if not obj.empty:
            groupby_order = np.concatenate(
                list(
                    self._grouper.indices.values())).astype(
                np.int64)
...
class Rolling(RollingAndExpandingMixin):
    ...
    def _validate_datetimelike_monotonic(self) -> None:
        ...
        if not (
                self._on.is_monotonic_increasing or self._on.is_monotonic_decreasing):
