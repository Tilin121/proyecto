...
def test_series_fast_transform_date():
    # GH 13191
    df = DataFrame({"grouping": [np.nan, 1, 1, 3],
                    "d": date_range("2014-1-1", "2014-1-4")})
...
@pytest.mark.parametrize("gb_target",
                         [{"by": np.random.default_rng(2).integers(0,
                                                                   50,
                                                                   size=10).astype(float)},
                          {"level": 0},
                             {"by": "string"},
                             pytest.param({"by": "string_missing"},
...
def test_transform_numeric_ret(cols, expected, agg_func):
    ...
    df = DataFrame({"a": date_range("2018-01-01", periods=3),
...
def test_categorical_and_not_categorical_key(observed):
    ...
    # Series case
    result = df_with_categorical.groupby(["A", "C"], observed=observed)[
        "B"].transform("sum")
