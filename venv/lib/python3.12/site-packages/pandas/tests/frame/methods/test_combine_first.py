...
class TestDataFrameCombineFirst:
    ...
    def test_combine_first_doc_example(self):
        # doc example
        df1 = DataFrame({"A": [1.0, np.nan, 3.0, 5.0, np.nan], "B": [
            np.nan, 2.0, 3.0, np.nan, 6.0]})
...
def test_combine_first_with_nan_multiindex():
    ...
    mi1 = MultiIndex.from_arrays([["b", "b", "c", "a", "b", np.nan], [
...
def test_combine_first_duplicates_rows_for_nan_index_values():
    # GH39881
    df1 = DataFrame({"x": [9, 10, 11]}, index=MultiIndex.from_arrays(
        [[1, 2, 3], [np.nan, 5, 6]], names=["a", "b"]), )

    df2 = DataFrame({"y": [12, 13, 14]}, index=MultiIndex.from_arrays(
