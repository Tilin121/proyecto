...
class TestReindexSetIndex:
    ...
    def test_dti_set_index_reindex_freq_with_tz(self):
        ...
        new_index = date_range(
            datetime(
                2015, 10, 2), datetime(
                2015, 10, 2, 23), freq="h", tz="US/Eastern")
    ...
    def test_reindex_timestamp_with_fold(
        ...
        df = (DataFrame({"index": [transition_1, transition_2], "vals": [
            "a", "b"]}) .set_index("index") .reindex(["1", "2"]))
        exp = DataFrame({"index": ["1", "2"], "vals": [
                        np.nan, np.nan]}).set_index("index")
...
class TestDataFrameSelectReindex:
    ...
    def test_reindex_frame_tz_ffill_bfill(
        ...
        # GH#38566
        obj = frame_or_series([0, 1, 2, 3], index=date_range(
            "2020-01-01 00:00:00", periods=4, freq="h", tz="UTC"), )
        new_index = date_range(
            "2020-01-01 00:01:00",
            periods=4,
            freq="h",
    ...
    def test_reindex_boolean(self):
        frame = DataFrame(
            np.ones(
                (10, 2), dtype=bool), index=np.arange(
                0, 20, 2), columns=[
                0, 2])
    ...
    def test_reindex_with_categoricalindex(self):
        ...
        expected = DataFrame(
            {"A": [0, 1, np.nan], "B": Series(list("abe"))}).set_index("B")
        ...
        expected = DataFrame({"A": [0, np.nan], "B": Series(
            list("ae")).astype(CategoricalDtype(cats))}).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)
        ...
        expected = DataFrame({"A": [0, np.nan], "B": Series(list("ae")).astype(
            CategoricalDtype(cats, ordered=True)), }).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)
