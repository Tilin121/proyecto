...
class ParserBase:
    ...
    def _get_complex_date_index(self, data, col_names):
        def _get_name(icol):
            ...
            if col_names is None:
                raise ValueError(
                    f"Must supply column order to use {
                        icol!s} as index")
    ...
    def _agg_index(self, index, try_parse_dates: bool = True) -> Index:
        ...
        for i, arr in enumerate(index):
            if try_parse_dates and self._should_parse_dates(i):
                arr = self._date_conv(
                    arr, col=self.index_names[i] if self.index_names is not None else None, )
    ...
    def _get_empty_meta(self, columns, dtype: DtypeArg | None = None):
        ...
        col_dict = {
            col_name: Series(
                [],
                dtype=dtype_dict[col_name]) for col_name in columns}
