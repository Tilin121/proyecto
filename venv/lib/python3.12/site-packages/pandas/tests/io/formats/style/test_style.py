    ...
    for attr in exp_attrs:
        ...
        assert not (
            all(check) if (
                hasattr(
                    check,
                    "__iter__") and len(check) > 0) else check)
...
def test_apply_map_header(method, axis):
    ...
    func = {
        "apply": lambda s: [
            "attr: val" if (
                "A" in v or "C" in v) else "" for v in s],
        "map": lambda v: "attr: val" if (
            "A" in v or "C" in v) else "",
    }
...
class TestStyler:
    ...
    def test_map_subset_multiindex_code(self):
        ...
        df = DataFrame([[1, -1, 1, 1], [-1, 1, 1, 1]],
                       index=["hello", "world"], columns=columns)
    ...
    def test_duplicate(self):
        ...
        result = s._translate(True, True)["cellstyle"]
        expected = [{"props": [("color", "red")], "selectors": [
            "row0_col0", "row1_col0"]}]
        assert result == expected
    ...
    def test_set_data_classes_reindex(self):
        # GH 39317
        df = DataFrame(data=[[0, 1, 2], [3, 4, 5], [6, 7, 8]], columns=[
            0, 1, 2], index=[0, 1, 2])
        classes = DataFrame(
            data=[["mi", "ma"], ["mu", "mo"]],
            columns=[0, 2],
...
def test_row_trimming_hide_index_mi():
    ...
    assert {"value": 0, "attributes": 'rowspan="2"',
            "is_visible": True}.items() <= ctx["body"][0][0].items()
    assert {
        "value": 0,
        "attributes": "",
        "is_visible": False}.items() <= ctx["body"][1][0].items()
