...
def _maybe_cast_to_extension_array(
        ...
        obj: ArrayLike,
        dtype: ExtensionDtype | None = None) -> ArrayLike:
    """
...
def convert_dtypes(
    ...
    if dtype_backend == "pyarrow":
        ...
        if (
            ...
            if isinstance(
                    inferred_dtype,
                    PandasExtensionDtype) and not isinstance(
                    inferred_dtype,
                    DatetimeTZDtype):
        ...
        if tipo is not None:
            if tipo.kind not in "iu":
                if isinstance(element,
                              np.ndarray) and element.dtype.kind == "f":
...
def _dtype_can_hold_range(rng: range, dtype: np.dtype) -> bool:
    ...
    return np_can_cast_scalar(
        rng.start, dtype) and np_can_cast_scalar(
        rng.stop, dtype)
