...
class TestBlockManager:
    ...
    def test_set_change_dtype(self, mgr):
        ...
        mgr2.iset(mgr2.items.get_loc("quux"),
...
class TestIndexing:
    ...
    def test_reindex_indexer(self, fill_value, mgr):
        ...
        for ax in range(mgr.ndim):
            ...
            assert_reindex_indexer_is_ok(
                mgr, ax, mgr.axes[ax], np.arange(
                    mgr.shape[ax])[
                    ::-1], fill_value)
            assert_reindex_indexer_is_ok(mgr, ax, Index(
                ["foo", "bar", "baz"]), np.array([0, 0, 0]), fill_value)
            assert_reindex_indexer_is_ok(mgr, ax, Index(
                ["foo", "bar", "baz"]), np.array([-1, 0, -1]), fill_value)
            assert_reindex_indexer_is_ok(
                mgr,
                ax,
