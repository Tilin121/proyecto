...
class TestPivotTable:
    ...
    def test_pivot_table_categorical_observed_equal(self, observed):
        # issue #24923
        df = DataFrame({"col1": list("abcde"), "col2": list(
    ...
    def test_pivot_no_values(self):
        ...
        exp = DataFrame([3.25, 2.0], index=Index(
            [1, 2], dtype=np.int32), columns=exp_columns)
        tm.assert_frame_equal(res, exp)
    ...
    def test_pivot_periods(self, method):
        ...
        exp_col = MultiIndex.from_arrays([exp_col1, exp_col2])
        expected = DataFrame([[0, 2, 0, 2], [1, 3, 1, 3]], index=pd.PeriodIndex(
            ["2013-01-01", "2013-01-02"], name="p1", freq="D"), columns=exp_col, )
        if method:
    ...
    def test_pivot_timegrouper_double(self):
        ...
        result = pivot_table(
            df, index=["Branch"], columns=[
                Grouper(
                    freq="ME", key="Date"), Grouper(
                    freq="ME", key="PayDay")], values="Quantity", aggfunc="sum", )
        tm.assert_frame_equal(result, expected.T)
    ...
    def test_pivot_with_categorical(self, observed, ordered):
        ...
        df = DataFrame(
            {
                "In": Categorical(
                    idx, categories=[
                        "low", "high"], ordered=ordered), "Col": Categorical(
                    col, categories=[
                        "A", "B"], ordered=ordered), "Val": range(
                            1, 6), })
        ...
        expected.index = Index(Categorical(["low", "high"], categories=[
            "low", "high"], ordered=ordered), name="In", )
    ...
    def test_pivot_string_as_func(self):
        ...
        mi = MultiIndex(levels=[["C"], ["one", "two"]], codes=[
            [0, 0], [0, 1]], names=[None, "B"])
        expected = DataFrame({("C", "one"): {"bar": 15, "foo": 13}, ("C", "two"): {
            "bar": 7, "foo": 20}}, columns=mi, ).rename_axis("A")
        tm.assert_frame_equal(result, expected)
