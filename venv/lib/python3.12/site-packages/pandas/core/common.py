...
def asarray_tuplesafe(
        values: Iterable,
        dtype: NpDtype | None = ...) -> ArrayLike:
    ...


def asarray_tuplesafe(
        values: Iterable,
        dtype: NpDtype | None = None) -> ArrayLike:
...
@overload
def random_state(state: int | np.ndarray | np.random.BitGenerator |
                 np.random.RandomState | None, ) -> np.random.RandomState:
    ...
...
def fill_missing_names(names: Sequence[Hashable | None]) -> list[Hashable]:
    ...
    """
    return [
        f"level_{i}" if name is None else name for i,
