...
def test_quantile(interpolation, a_vals, b_vals, q, request):
    ...
    expected = DataFrame([a_expected, b_expected], columns=[
        "val"], index=Index(["a", "b"], name="key"))
...
def test_quantile_array():
    ...
    expected = DataFrame({"A": [0.25, 0.75, 2.25, 2.75], "B": [
        4.25, 4.75, 6.25, 6.75]}, index=index)
    tm.assert_frame_equal(result, expected)
...
def test_quantile_array_multiple_levels():
    df = DataFrame({"A": [0, 1, 2], "B": [3, 4, 5], "c": [
        "a", "a", "a"], "d": ["a", "a", "b"]})
...
def test_groupby_quantile_with_arraylike_q_and_int_columns(
    ...
    expected_values = [[float(x)] * (ncol - len(groupby))
                       for x in range(min(nrow, 4)) for _ in q]
...
def test_groupby_quantile_NA_int(any_int_ea_dtype):
    ...
    expected = DataFrame({"y": 3.5}, dtype="Float64", index=Index(
        [1], name="x", dtype=any_int_ea_dtype))
    tm.assert_frame_equal(result, expected)


@pytest.mark.parametrize("interpolation, val1, val2",
...
def test_groupby_timedelta_quantile():
    # GH: 29485
    df = DataFrame({"value": pd.to_timedelta(
        np.arange(4), unit="s"), "group": [1, 1, 2, 2]})
