...
def string_column_to_ndarray(col: Column) -> tuple[np.ndarray, Any]:
    ...
    # We're going to reinterpret the buffer as uint8, so make sure we can do
    # it safely
    ...
    # an equivalent ndarray, we claim that the buffer is uint8 (i.e., a byte
    # array)
    data_dtype = (
        DtypeKind.UINT,
        8,
...
def datetime_column_to_ndarray(
        col: Column) -> tuple[np.ndarray | pd.Series, Any]:
    """
    ...
    dbuf, _ = buffers["data"]
    # Consider dtype being `uint` to get number of units passed since the
    # 01.01.1970
