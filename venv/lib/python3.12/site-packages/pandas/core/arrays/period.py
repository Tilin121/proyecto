...
class PeriodArray(
        dtl.DatelikeOps,
        libperiod.PeriodMixin):  # type: ignore[misc]
    """
        ...
        if type is not None:
            if pyarrow.types.is_integer(type):
                ...
                if self.freqstr != type.freq:
                    raise TypeError(
                        "Not supported to convert PeriodArray to array with different " f"'freq' ({
                            self.freqstr} vs {
                            type.freq})")
            else:
                raise TypeError(
    ...
    def _add_timedelta_arraylike(
        ...
        if not self.dtype._is_tick_like():
            # We cannot add timedelta-like to non-tick PeriodArray
            raise TypeError(
                f"Cannot add or subtract timedelta64[ns] dtype from {
                    self.dtype}")
