...
def concatenate_managers(
    ...
    if len(mgrs_indexers) > 0 and mgrs_indexers[0][0].nblocks > 0:
        ...
        if first_dtype in [np.float64, np.float32]:
            ...
            if (all(_is_homogeneous_mgr(mgr, first_dtype)
                    for mgr, _ in mgrs_indexers) and len(mgrs_indexers) > 1):
                # Fastpath!
    ...
    if empty_dtype != empty_dtype_future:
        if empty_dtype == concat_values.dtype:
            ...
            warnings.warn(
                ...
                "the concat operation.", FutureWarning, stacklevel=find_stack_level(), )
