...
def test_closed_fixed_binary_col(center, step):
    ...
    expected = DataFrame(expected_data, columns=["binary_col"], index=date_range(
        start="2020-01-01", freq="min", periods=len(expected_data)), )[::step]
...
def test_missing_minp_zero_variable():
    # https://github.com/pandas-dev/pandas/pull/18921
    x = Series([np.nan] * 4, index=DatetimeIndex(["2017-01-01",
                                                  "2017-01-04", "2017-01-06", "2017-01-07"]), )
...
def test_iter_rolling_on_dataframe(expected, window):
    ...
    expected = [DataFrame(values, index=df.loc[index, "C"])
...
def test_rolling_numeric_dtypes():
    # GH#41779
    df = DataFrame(np.arange(40).reshape(4,
                                         10),
                   columns=list("abcdefghij")).astype({"a": "float16",
                                                       "b": "float32",
                                                       "c": "float64",
                                                       "d": "int8",
                                                       "e": "int16",
...
def test_rolling_quantile_np_percentile():
    ...
    df = DataFrame(np.random.default_rng(2).random(
        row * col).reshape((row, -1)), index=idx)
...
def test_rolling_timedelta_window_non_nanoseconds(unit, tz):
    # Test Sum, GH#55106
    df_time = DataFrame({"A": range(5)}, index=date_range(
        "2013-01-01", freq="1s", periods=5, tz=tz))
