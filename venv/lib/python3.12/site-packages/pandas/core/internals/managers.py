...
class BlockManager(libinternals.BlockManager, BaseBlockManager):
            ...
            for block in blocks:
                if self.ndim != block.ndim:
                    raise AssertionError(
                        f"Number of Block dimensions ({
                            block.ndim}) must equal " f"number of axes ({
                            self.ndim})")
    ...
    def iset(
        ...
        if len(removed_blknos):
            ...
            self.blocks = tuple(blk for i, blk in enumerate(
                self.blocks) if i not in set(removed_blknos))
    ...
    def _iset_split_block(
        ...
        blocks_tup = (self.blocks[:blkno_l] + (first_nb,) +
    ...
    def insert(
        ...
        if value.ndim == 2:
            ...
            if len(value) > 1:
                raise ValueError(
                    f"Expected a 1D array, got an array with shape {
                        value.T.shape}")
        else:
    ...
    def quantile(
        ...
        *,
        ...
        blocks = [blk.quantile(qs=qs, interpolation=interpolation)
                  for blk in self.blocks]
...
class SingleBlockManager(BaseBlockManager, SingleDataManager):
    ...
    def from_array(
            ...
            array: ArrayLike,
            index: Index,
            refs: BlockValuesRefs | None = None) -> SingleBlockManager:
        """
...
def _tuples_to_blocks_no_consolidate(tuples, refs) -> list[Block]:
    ...
    return [
        new_block_2d(
            ensure_block_shape(
                arr, ndim=2), placement=BlockPlacement(i), refs=ref) for (
            (i, arr), ref) in zip(
                    tuples, refs)]
