...
class TestFrameComparisons:
    ...
    def test_timestamp_compare(self, left, right):
        ...
        # GH#4982
        df = DataFrame({"dates1": pd.date_range("20010101",
                                                periods=10),
                        "dates2": pd.date_range("20010102",
                                                periods=10),
                        "intcol": np.random.default_rng(2).integers(1000000000,
                                                                    size=10),
...
class TestFrameFlexArithmetic:
    ...
    def test_frame_multiindex_operations(self):
        ...
        series = Series([0.4], index=MultiIndex.from_product(
            [["b"], ["a"]], names=["mod", "scen"]), )
    ...
    def test_frame_multiindex_operations_series_index_to_frame_index(self):
        # GH 43321
        df = DataFrame({2010: [1], 2020: [3]}, index=MultiIndex.from_product(
            [["a"], ["b"]], names=["scen", "mod"]), )
...
class TestFrameArithmetic:
    ...
    @pytest.mark.parametrize("values", [[1, 2], (1, 2),
                                        np.array([1, 2]), range(1, 3), deque([1, 2])])
...
def test_frame_with_zero_len_series_corner_cases():
    ...
    # easy all-float case
    df = DataFrame(
        np.random.default_rng(2).standard_normal(6).reshape(
            3, 2), columns=[
            "A", "B"])
