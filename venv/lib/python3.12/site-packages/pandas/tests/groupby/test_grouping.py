...
class TestGrouping:
    ...
    def test_grouper_multilevel_freq(self):
        ...
        expected = (df.reset_index() .groupby(
            [Grouper(key="foo", freq="W"), Grouper(key="bar", freq="W")]) .sum())
    ...
    def test_grouper_column_and_index(self):
        ...
        expected = (df_multi.reset_index().groupby(
        ...
        expected = (df_single.reset_index().groupby(
            ["B", "inner"]).mean(numeric_only=True))
        tm.assert_frame_equal(result, expected)
    ...
    def test_groupby_level_index_names(self, axis):
        ...
        msg = f"level name foo is not the name of the {
            df._get_axis_name(axis)}"
        with pytest.raises(ValueError, match=msg):
            with tm.assert_produces_warning(FutureWarning, match=depr_msg):
    ...
    @pytest.mark.parametrize(
        "func,expected", [
            ("transform", Series(
                name=2, dtype=np.float64), ), ("agg", Series(
                    name=2, dtype=np.float64, index=Index(
                        [], dtype=np.float64, name=1)), ), ("apply", Series(
                            name=2, dtype=np.float64, index=Index(
                                [], dtype=np.float64, name=1)), ), ], )
            ...
            data=[], index=MultiIndex(
                levels=[
                    Index(
                        ...
                        [], dtype="float64")], codes=[[], []], names=[
                    "A", "B"], ), columns=["C"], dtype="int64", )
        tm.assert_frame_equal(result, expected)
