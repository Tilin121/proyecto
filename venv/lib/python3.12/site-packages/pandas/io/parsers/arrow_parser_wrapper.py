        ...
        if on_bad_lines is not None:
            if callable(on_bad_lines):
                ...
                def handle_warning(invalid_row) -> str:
                    warnings.warn(
                        f"Expected {
                            invalid_row.expected_columns} columns, but found " f"{
                            invalid_row.actual_columns}: {
                            invalid_row.text}",
                        ParserWarning,
        ...
        if self.index_col is not None:
            ...
            for i, item in enumerate(self.index_col):
                ...
                if self.dtype is not None:
                    key, new_dtype = (
                        (item, self.dtype.get(item)) if self.dtype.get(item) is not None else (
                            frame.columns[item], self.dtype.get(
                                frame.columns[item])))
