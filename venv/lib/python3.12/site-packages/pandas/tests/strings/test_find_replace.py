...
def test_contains(any_string_dtype):
    values = np.array(["foo", np.nan, "fooommm__foo",
...
def test_startswith_nullable_string_dtype(nullable_string_dtype, na):
    ...
    exp = Series([False, na, True, False, False, na,
                  True, False, False], dtype="boolean")
    tm.assert_series_equal(result, exp)

    result = values.str.startswith("rege.", na=na)
    exp = Series([False, na, False, False, False, na,
                  False, False, True], dtype="boolean")
    tm.assert_series_equal(result, exp)
...
def test_endswith_nullable_string_dtype(nullable_string_dtype, na):
    ...
    exp = Series([False, na, False, False, True, na,
                  True, False, False], dtype="boolean")
    tm.assert_series_equal(result, exp)
...
def test_replace_mixed_object():
    ser = Series(["aBAD", np.nan, "bBAD", True,
                  datetime.today(), "fooBAD", None, 1, 2.0])
...
def test_match(any_string_dtype):
    ...
    values = Series(["fooBAD__barBAD", "BAD_BADleroybrown",
...
def test_fullmatch(any_string_dtype):
    # GH 32806
    ser = Series(["fooBAD__barBAD", "BAD_BADleroybrown",
                  np.nan, "foo"], dtype=any_string_dtype)
...
def test_find(any_string_dtype):
    ser = Series(["ABCDEFG", "BCDEFEF", "DEFGHIJEF",
                  "EFGHEF", "XXXX"], dtype=any_string_dtype)
...
def test_find_nan(any_string_dtype):
    ser = Series(["ABCDEFG", np.nan, "DEFGHIJEF",
                  np.nan, "XXXX"], dtype=any_string_dtype)
