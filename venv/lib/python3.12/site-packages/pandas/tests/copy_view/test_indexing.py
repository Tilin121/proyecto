...
def test_subset_set_with_row_indexer(
    ...
    expected = DataFrame({"a": [0, 0, 4], "b": [0, 0, 7], "c": [
        0.0, 0.0, 0.4]}, index=range(1, 4))
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
        # original parent dataframe is not modified (CoW)
        tm.assert_frame_equal(df, df_orig)
...
def test_subset_set_with_mask(
    ...
    expected = DataFrame({"a": [2, 3, 0], "b": [0, 0, 0], "c": [
        0.20, 0.3, 0.4]}, index=range(1, 4))
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
        # original parent dataframe is not modified (CoW)
        tm.assert_frame_equal(df, df_orig)
...
def test_series_midx_slice(using_copy_on_write, warn_copy_on_write):
    ...
    if using_copy_on_write:
        ...
        expected = Series([100, 2, 3], index=pd.MultiIndex.from_arrays(
            [[1, 1, 2], [3, 4, 5]]))
...
def test_series_midx_tuples_slice(using_copy_on_write, warn_copy_on_write):
    ...
    with tm.assert_cow_warning(warn_copy_on_write):
        result.iloc[0] = 100
    if using_copy_on_write:
        expected = Series([1, 2, 3], index=pd.MultiIndex.from_tuples(
            [((1, 2), 3), ((1, 2), 4), ((2, 3), 4)]), )
        tm.assert_series_equal(ser, expected)
