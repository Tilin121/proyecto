...
def test_center(roll_func, kwargs, minp):
    ...
    result = getattr(
        ...
        roll_func)(
        **kwargs)
    expected = (getattr(concat([obj,
                                Series([np.nan] * 9)]).rolling(20,
                                                               min_periods=minp),
...
def test_rolling_max_gh6297(step):
    ...
    expected = Series([1.0, 2.0, 6.0, 4.0, 5.0], index=DatetimeIndex(
        [datetime(1975, 1, i, 0) for i in range(1, 6)], freq="D"), )[::step]
...
def test_rolling_max_resample(step):
    ...
    expected = Series([0.0, 1.0, 2.0, 3.0, 20.0], index=DatetimeIndex(
        [datetime(1975, 1, i, 0) for i in range(1, 6)], freq="D"), )[::step]
    x = series.resample("D").max().rolling(window=1, step=step).max()
    tm.assert_series_equal(expected, x)

    # Now specify median (10.0)
    expected = Series([0.0, 1.0, 2.0, 3.0, 10.0], index=DatetimeIndex(
        [datetime(1975, 1, i, 0) for i in range(1, 6)], freq="D"), )[::step]
    ...
    v = (4.0 + 10.0 + 20.0) / 3.0
    expected = Series([0.0, 1.0, 2.0, 3.0, v], index=DatetimeIndex(
        [datetime(1975, 1, i, 0) for i in range(1, 6)], freq="D"), )[::step]
...
def test_rolling_min_resample(step):
    ...
    expected = Series([0.0, 1.0, 2.0, 3.0, 4.0], index=DatetimeIndex(
        [datetime(1975, 1, i, 0) for i in range(1, 6)], freq="D"), )[::step]
...
def test_rolling_median_resample():
    ...
    expected = Series([0.0, 1.0, 2.0, 3.0, 10], index=DatetimeIndex(
