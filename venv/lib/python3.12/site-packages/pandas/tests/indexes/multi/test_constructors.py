...
def test_constructor_single_level():
    result = MultiIndex(levels=[["foo", "bar", "baz", "qux"]], codes=[
        [0, 1, 2, 3]], names=["first"])
...
def test_na_levels():
    ...
    expected = MultiIndex(levels=[[np.nan, "s", pd.NaT, 128, None]], codes=[
        [-1, -1, 1, -1, 3, -1]])
    tm.assert_index_equal(result, expected)
...
@pytest.mark.parametrize(("idx1",
                          "idx2"),
                         [(pd.period_range("2011-01-01",
                                           freq="D",
                                           periods=3),
                           pd.period_range("2015-01-01",
                                           freq="h",
                                           periods=3),
                           ),
                          (date_range("2015-01-01 10:00",
                                      freq="D",
                                      periods=3,
                                      tz="US/Eastern"),
                             date_range("2015-01-01 10:00",
                          ...
                          (pd.timedelta_range("1 days",
                                              freq="D",
                                              periods=3),
                             pd.timedelta_range("2 hours",
                                                freq="h",
                                                periods=3),
                           ),
                          ],
...
def test_from_arrays_respects_none_names():
    ...
    expected = MultiIndex(levels=[[1, 2, 3], ["a", "b", "c"]], codes=[
        [0, 1, 2], [0, 1, 2]], names=None)
...
def test_raise_invalid_sortorder():
    ...
    # Correct sortorder
    MultiIndex(levels=levels, codes=[
        [0, 0, 0, 1, 1, 1], [0, 1, 2, 0, 1, 2]], sortorder=2)

    with pytest.raises(ValueError, match=r".* sortorder 2 with lexsort_depth 1.*"):
        MultiIndex(levels=levels, codes=[
            [0, 0, 0, 1, 1, 1], [0, 1, 2, 0, 2, 1]], sortorder=2)

    with pytest.raises(ValueError, match=r".* sortorder 1 with lexsort_depth 0.*"):
        MultiIndex(levels=levels, codes=[
            [0, 0, 1, 0, 1, 1], [0, 1, 0, 2, 2, 1]], sortorder=1)


def test_datetimeindex():
    idx1 = pd.DatetimeIndex(["2013-04-01 9:00",
                             "2013-04-02 9:00",
                             "2013-04-03 9:00"] * 2,
                            tz="Asia/Tokyo")
