...
class TestTimeConversionFormats:
    ...
    def test_to_datetime_format_YYYYMMDD_with_nat(self, cache):
        ...
        # with NaT
        expected = Series([Timestamp("19801222"), Timestamp(
            "19801222")] + [Timestamp("19810105")] * 5)
        expected[2] = np.nan
        ser[2] = np.nan
    ...
    def test_to_datetime_format_YYYYMM_with_nat(self, cache):
        ...
        expected = Series([Timestamp("19801201"), Timestamp(
            "19801201")] + [Timestamp("19810101")] * 5)
        expected[2] = np.nan
        ser[2] = np.nan
...
class TestToDatetime:
    ...
    @pytest.mark.parametrize("fmt, expected",
                             [pytest.param("%Y-%m-%d %H:%M:%S%z",
                                           DatetimeIndex(["2000-01-01 08:00:00+00:00",
    ...
    def test_to_datetime_now_with_format(
        ...
        expected = DatetimeIndex([expected_ds, getattr(
            Timestamp, attribute)()], dtype="datetime64[ns, UTC]")
    ...
    def test_to_datetime_array_of_dt64s(self, cache, unit):
        ...
        if not cache:
            ...
            expected = DatetimeIndex([Timestamp(dts_with_oob[0]).asm8, Timestamp(
                dts_with_oob[1]).asm8] * 30 + [NaT], )
        else:
...
class TestToDatetimeUnit:
    ...
    def test_to_datetime_unit_with_nulls(self, null):
        ...
        result = to_datetime(ser, unit="s")
        expected = Series([Timestamp("2013-06-09 02:42:28") +
                           timedelta(seconds=t) for t in range(20)] +
                          [NaT], dtype="M8[ns]", )
        tm.assert_series_equal(result, expected)
...
class TestToDatetimeDataFrame:
    ...
    def test_dataframe_duplicate_columns_raises(self, cache):
        ...
        df2 = DataFrame({"year": [2015, 2016], "month": [
            2, 20], "day": [4, 5], "hour": [4, 5]})
...
class TestToDatetimeMisc:
    ...
    def test_string_na_nat_conversion_with_name(self, cache):
        idx = ["a", "b", "c", "d", "e"]
        series = Series(["1/1/2000", np.nan, "1/3/2000",
                         np.nan, "1/5/2000"], index=idx, name="foo")
        dseries = Series(
            [
...
class TestToDatetimeInferFormat:
    @pytest.mark.parametrize("test_format",
                             ["%m-%d-%Y",
                              "%m/%d/%Y %H:%M:%S.%f",
                              "%Y-%m-%dT%H:%M:%S.%f"])
...
class TestOrigin:
    ...
    def test_epoch(self, units, epochs):
        ...
        expected = Series([pd.Timedelta(x, unit=units) +
...
class TestShouldCache:
    ...
    @pytest.mark.parametrize("unique_share,check_count, err_message",
                             [(0.5,
                              ...
                              (10,
                               2,
                               r"unique_share must be in next bounds: \(0; 1\)"),
                              ],
                             )
...
def test_to_datetime_cache_coerce_50_lines_outofbounds(series_length):
    ...
    ser = Series([datetime.fromisoformat("1446-04-12 00:00:00+00:00")] +
                 ([datetime.fromisoformat("1991-10-20 00:00:00+00:00")] *
                  series_length), dtype=object, )
    ...
    expected2 = Series([datetime.fromisoformat("1446-04-12 00:00:00+00:00")] + (
...
def test_to_datetime_with_empty_str_utc_false_offsets_and_format_mixed():
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        to_datetime(["2020-01-01 00:00+00:00",
                     "2020-01-01 00:00+02:00", ""], format="mixed")
...
@pytest.mark.parametrize("naive_val",
                         [dtstr[:-6],
                          ts.tz_localize(None),
                          ts.date(),
                          ts.asm8,
