...
7,8,NaN
"""
    expected = DataFrame([[1.0, np.nan, 3], [np.nan, 5, np.nan], [
                         ...
                         [([0],
                           DataFrame({"b": [np.nan],
                             ...
                             ([0,
                              ...
                              DataFrame({"b": [np.nan],
                                         "d": [5]},
                                        index=MultiIndex.from_tuples([(0,
                                                                       1)],
                                                                     names=["a",
                                                                     "c"]),
                                        ),
...
def test_inf_na_values_with_int_index(all_parsers):
    ...
    out = parser.read_csv(
        ...
        na_values=[
            "inf",
            "-inf"])
    expected = DataFrame({"col1": [3, np.nan], "col2": [
        4, np.nan]}, index=Index([1, 2], name="idx"))
...
def test_nan_multi_index(all_parsers):
    ...
    if parser.engine == "pyarrow":
        msg = "The pyarrow engine doesn't support passing a dict for na_values"
        with pytest.raises(ValueError, match=msg):
            parser.read_csv(StringIO(data), header=list(
                range(2)), na_values={("B", "Z"): "inf"})
        return
