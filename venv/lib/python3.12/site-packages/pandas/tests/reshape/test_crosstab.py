...
class TestCrosstab:
    ...
    def test_crosstab_pass_values(self):
        ...
        table = crosstab([a, b], c, values, aggfunc="sum",
    ...
    def test_margin_dropna6(self):
        ...
        actual = crosstab(a, [b, c], rownames=["a"], colnames=[
            "b", "c"], margins=True, dropna=False)
        ...
        expected = DataFrame([[1, 0, 1, 0, 0, 0, 2], [2, 0, 1, 1, 0, 1, 5], [
            3, 0, 2, 1, 0, 0, 7]], columns=m, )
        expected.index = Index(["bar", "foo", "All"], name="a")
        tm.assert_frame_equal(actual, expected)

        actual = crosstab([a, b], c, rownames=["a", "b"], colnames=[
            "c"], margins=True, dropna=False)
        m = MultiIndex.from_arrays(
            [
    ...
    def test_crosstab_normalize(self):
        # Issue 12578
        df = DataFrame({"a": [1, 2, 2, 2, 2], "b": [
            3, 3, 4, 4, 4], "c": [1, 1, np.nan, 1, 1]})
    ...
    def test_crosstab_normalize_arrays(self):
        ...
        # Test arrays
        crosstab([np.array([1, 1, 2, 2]), np.array(
    ...
    def test_margin_normalize(self):
        ...
        # normalize on index
        result = crosstab([df.A, df.B], df.C, margins=True,
                          margins_name="Sub-Total", normalize=0)
        expected = DataFrame([[0.5, 0.5], [0.5, 0.5], [0.666667, 0.333333], [
        ...
        result = crosstab([df.A, df.B], df.C, margins=True,
                          margins_name="Sub-Total", normalize=1)
        expected = DataFrame(
            [
                [0.25, 0.2, 0.222222],
                [0.25, 0.2, 0.222222],
        ...
        result = crosstab([df.A, df.B], df.C, margins=True,
                          margins_name="Sub-Total", normalize=True)
        expected = DataFrame(
            [
