...
class TestBasic(Base):
    ...
    def test_multiindex_with_columns(self, pa):
        ...
        for index in [index1, index2]:
            ...
            check_round_trip(df, engine)
            check_round_trip(df, engine, read_kwargs={"columns": [
                "A", "B"]}, expected=df[["A", "B"]])
...
class TestParquetPyArrow(Base):
    ...
    def test_categorical(self, pa):
        ...
        df["c"] = pd.Categorical(["a", "b", "c", "a", "c", "b"], categories=[
            "b", "c", "d"], ordered=True)
    ...
    def test_read_dtype_backend_pyarrow_config_index(self, pa):
        df = pd.DataFrame({"a": [1, 2]}, index=pd.Index(
            [3, 4], name="test"), dtype="int64[pyarrow]")
        expected = df.copy()
        import pyarrow
