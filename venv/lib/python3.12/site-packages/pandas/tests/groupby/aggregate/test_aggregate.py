...
def test_groupby_aggregation_multi_level_column():
    ...
    df = DataFrame(data=lst, columns=MultiIndex.from_tuples(
        [("A", 0), ("A", 1), ("B", 0), ("B", 1)]), )
...
def test_callable_result_dtype_frame(
    ...
    expected = DataFrame({"c": [df["c"].iloc[0]]},
                         index=expected_index).astype(result_dtype)
    if method == "apply":
...
class TestNamedAggregationSeries:
    def test_series_named_agg(self):
        ...
        expected = DataFrame({"a": [3, 7], "b": [1, 3]}, columns=[
            "a", "b"], index=np.array([0, 1]))
        tm.assert_frame_equal(result, expected)
...
def test_grouby_agg_loses_results_with_as_index_false_relabel():
    ...
    df = DataFrame({"key": ["x", "y", "z", "x", "y", "z"], "val": [
        1.0, 0.8, 2.0, 3.0, 3.6, 0.75]})
...
def test_grouby_agg_loses_results_with_as_index_false_relabel_multiindex():
    ...
    result = grouped.agg(min_val=pd.NamedAgg(column="val", aggfunc="min"))
    expected = DataFrame({"key": ["x", "x", "y"], "key1": [
...
class TestLambdaMangling:
    ...
    def test_agg_multiple_lambda(self):
        ...
        result2 = df.groupby(
            ...
            height_sqr_min=pd.NamedAgg(
                column="height",
                aggfunc=lambda x: np.min(
                    x**2)),
            height_max=pd.NamedAgg(
                column="height",
                aggfunc="max"),
            weight_max=pd.NamedAgg(
                column="weight",
                aggfunc="max"),
...
def test_series_groupy_agg_list_like_func_with_args():
    ...
    expected = DataFrame([[8, 8], [9, 9], [10, 10]], index=Index(
        [1, 2, 3]), columns=["foo1", "foo2"])
    tm.assert_frame_equal(result, expected)
...
def test_groupby_aggregation_empty_group():
    ...
    df = DataFrame({"A": pd.Categorical(
        ["a", "a"], categories=["a", "b", "c"]), "B": [1, 1]})
