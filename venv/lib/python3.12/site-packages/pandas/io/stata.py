...
def _stata_elapsed_date_to_datetime_vec(dates: Series, fmt: str) -> Series:
    ...
    def convert_year_days_safe(year, days) -> Series:
        ...
        if year.max() < (MAX_YEAR - 1) and year.min() > MIN_YEAR:
            ...
            index = getattr(year, "index", None)
            value = [datetime(y, 1, 1) + timedelta(days=int(d))
...
def _cast_to_stata_types(data: DataFrame) -> DataFrame:
    ...
    for col in data:
        ...
        if dtype == np.int8 and not empty_df:
            ...
            if empty_df or (
                    data[col].max() <= 2147483620 and data[col].min() >= -
                    2147483647):
                data[col] = data[col].astype(np.int32)
            else:
...
class StataReader(StataParser, abc.Iterator):
    ...
    def _read_new_header(self) -> None:
        ...
        self._nvar = (self._read_uint16() if self._format_version <=
                      118 else self._read_uint32())
    ...
    def _get_variable_labels(self) -> list[str]:
        if self._format_version >= 118:
            vlblist = [
                self._decode(
                    self._path_or_buf.read(321)) for _ in range(
                    self._nvar)]
        elif self._format_version > 105:
            vlblist = [
                self._decode(
                    self._path_or_buf.read(81)) for _ in range(
                    self._nvar)]
        else:
            vlblist = [
                self._decode(
                    self._path_or_buf.read(32)) for _ in range(
                    self._nvar)]
        return vlblist
    ...
    def _read_old_header(self, first_char: bytes) -> None:
        ...
        if self._format_version > 108:
            self._varlist = [
                self._decode(
                    self._path_or_buf.read(33)) for _ in range(
                    self._nvar)]
        else:
            self._varlist = [
                self._decode(
                    self._path_or_buf.read(9)) for _ in range(
                    self._nvar)]
        self._srtlist = self._read_int16_count(self._nvar + 1)[:-1]
    ...
    def write_file(self) -> None:
        ...
        with get_handle(
            ...
            try:
                ...
                self.handles.close()
                if isinstance(
                        self._fname, (str, os.PathLike)) and os.path.isfile(
                        self._fname):
                    try:
                        os.unlink(self._fname)
                    except OSError:
                        warnings.warn(
                            f"This save was not successful but {
                                self._fname} could not " "be deleted. This file is not valid.",
                            ResourceWarning,
                            stacklevel=find_stack_level(),
                        )
                raise exc
