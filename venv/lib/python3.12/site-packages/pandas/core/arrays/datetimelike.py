...
class DatetimeLikeArrayMixin(  # type: ignore[misc]
    ...
    def _unbox(
            self,
            other) -> np.int64 | np.datetime64 | np.timedelta64 | np.ndarray:
        """
    ...
    def _sub_datetimelike_scalar(
        ...
        if self.dtype.kind != "M":
            raise TypeError(
                f"cannot subtract a datelike from a {
                    type(self).__name__}")
    ...
    def _accumulate(self, name: str, *, skipna: bool = True, **kwargs) -> Self:
        if name not in {"cummin", "cummax"}:
            raise TypeError(
                f"Accumulation {name} not supported for {
                    type(self)}")
    ...
    def __add__(self, other):
        ...
        if isinstance(
                result,
                np.ndarray) and lib.is_np_dtype(
                result.dtype,
                "m"):
            from pandas.core.arrays import TimedeltaArray
...
class TimelikeOps(DatetimeLikeArrayMixin):
    ...
    def __init__(
        ...
        if freq == "infer":
            raise ValueError(
                f"Frequency inference not allowed in {
                    type(self).__name__}.__init__. " "Use 'pd.array()' instead.")
