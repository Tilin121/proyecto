...
class TestAsOfMerge:
    def prep_data(self, df, dedupe=False):
        if dedupe:
            df = df.drop_duplicates(
                ["time", "ticker"], keep="last").reset_index(drop=True)
    ...
    def test_examples1(self):
        ...
        expected = pd.DataFrame({"a": [1, 5, 10], "left_val": [
    ...
    def test_allow_exact_matches_and_tolerance2(self):
        # GH 13695
        df1 = pd.DataFrame({"time": to_datetime(
            ["2016-07-15 13:30:00.030"]), "username": ["bob"]})
        df2 = pd.DataFrame(
            {
                "time": to_datetime(
    ...
    def test_int_type_tolerance(self, any_int_dtype):
        ...
        expected = pd.DataFrame({"a": [0, 10, 20], "left_val": [
            1, 2, 3], "right_val": [np.nan, 1.0, 2.0]})
...
def test_merge_asof_numeric_column_in_multiindex():
    # GH#34488
    left = pd.DataFrame({"b": [10, 11, 12]}, index=pd.MultiIndex.from_arrays(
        [[1, 2, 3], ["a", "b", "c"]], names=["a", "z"]), )
    right = pd.DataFrame({"c": [20, 21, 22]}, index=pd.MultiIndex.from_arrays(
