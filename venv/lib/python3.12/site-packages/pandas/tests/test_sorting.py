...
def left_right():
    ...
    left = DataFrame(np.random.default_rng(2).integers(
...
class TestSorting:
    ...
    def test_int64_overflow_groupby_large_df_shuffled(self, agg):
        ...
        mi = MultiIndex.from_arrays([ar.ravel() for ar in np.array_split(
            np.unique(arr, axis=0), 5, axis=1)], names=list("abcde"), )
...
class TestMerge:
    ...
    def test_int64_overflow_one_to_many_none_match(self, how, sort):
        ...
        left = DataFrame(np.random.default_rng(2).integers(
            low, high, (n, 7)).astype("int64"), columns=list("ABCDEFG"), )
        ...
        right = DataFrame(np.random.default_rng(3).integers(
            low, high, (n // 2, 7)).astype("int64"), columns=list("ABCDEFG"), )
...
class TestSafeSort:
    ...
    @pytest.mark.parametrize("arg, codes, err, msg",
                             [[1,
                               None,
                               TypeError,
                               "Only np.ndarray, ExtensionArray, and Index"],
                              [np.array([0,
                                         1,
                                         2]),
                               1,
