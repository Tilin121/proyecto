    ...
    def _getitem_lowerdim(self, tup: tuple):
        ...
        if (
            ...
            with suppress(IndexingError):
                return cast(
                    _LocIndexer,
                    self)._handle_lowerdim_multi_index_axis0(tup)
...
class _LocIndexer(_LocationIndexer):
    ...
    def _convert_to_indexer(self, key, axis: AxisInt):
        ...
        if is_scalar(key) or (isinstance(labels, MultiIndex)
                              and is_hashable(key) and not contains_slice):
...
class _iLocIndexer(_LocationIndexer):
    ...
    def _validate_key(self, key, axis: AxisInt):
        ...
        if isinstance(key, slice):
            ...
            raise ValueError(
                f"Can only index by location with a [{
                    self._valid_types}]")
...
class _AtIndexer(_ScalarAccessIndexer):
    ...
    def __getitem__(self, key):
        if self.ndim == 2 and not self._axes_are_unique:
            ...
            if not isinstance(
                    key, tuple) or not all(
                    is_scalar(x) for x in key):
                raise ValueError("Invalid call for scalar access (getting)!")
...
def convert_from_missing_indexer_tuple(indexer, axes):
    ...
    def get_indexer(_i, _idx):
        return axes[_i].get_loc(
            _idx["key"]) if isinstance(
            _idx,
            dict) else _idx
