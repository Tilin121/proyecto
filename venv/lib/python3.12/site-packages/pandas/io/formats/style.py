    ...
    *{"row": "MY_ROW_CLASS", "col_trim": "", "row_trim": ""}*.
        ...
        if data.empty:
            ...
            if not isinstance(result, DataFrame):
                ...
                if data.shape != result.shape:
                    raise ValueError(
                        f"Function {
                            repr(func)} returned ndarray with wrong shape.\n" f"Result has shape: {
                            result.shape}\n" f"Expected shape: {
                            data.shape}")
                result = DataFrame(
    ...
    def apply(
        ...
        **kwargs : dict
        ...
        more details.
        """
        self._todo.append((lambda instance: getattr(
            instance, "_apply"), (func, axis, subset), kwargs))
        return self
    ...
    def map(
        ...
        **kwargs : dict
        ...
        """
        self._todo.append((lambda instance: getattr(
            instance, "_map"), (func, subset), kwargs))
        return self
...
def _bar(
    ...
    def css_bar(start: float, end: float, color: str) -> str:
        ...
        if end > start:
            ...
            if start > 0:
                cell_css += f" transparent {start *
                                            100:.1f}%, {color} {start *
                                                                100:.1f}%,"
            cell_css += f" {color} {end *
                                    100:.1f}%, transparent {end *
                                                            100:.1f}%)"
        return cell_css
    ...
    def css_calc(
        ...
        if align == "left":
            ...
            if align == "zero":
                ...
                z_frac = (-mid / (right - left) + 0.5 if mid <
                          0 else -left / (right - left))
    ...
    if cmap is not None:
        ...
        with _mpl(Styler.bar) as (_, _matplotlib):
            ...
            if data.ndim == 1:
                ...
                rgbas = [[_matplotlib.colors.rgb2hex(
                    rgba) for rgba in row] for row in rgbas]
