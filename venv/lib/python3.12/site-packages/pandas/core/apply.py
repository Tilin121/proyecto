...
class Apply(metaclass=abc.ABCMeta):
    ...
    def _apply_str(self, obj, func: str, *args, **kwargs):
        ...
        if hasattr(obj, func):
            ...
            msg = f"'{func}' is not a valid function for '{
    ...
    def generate_numba_apply_func(
        ...
        def numba_func(values, col_names, df_index):
            ...
            for j in range(values.shape[1]):
                # Create the series
                ser = Series(values[:, j], index=df_index,
    ...
    def agg(self):
        ...
        if result is None:
            ...
            try:
                ...
                msg = (
                    f"using {func} in {
                        type(obj).__name__}.agg cannot aggregate and " f"has been deprecated. Use {
                        type(obj).__name__}.transform to " f"keep behavior unchanged.")
                warnings.warn(
                    msg,
                    FutureWarning,
                    stacklevel=find_stack_level())
...
def is_multi_agg_with_relabel(**kwargs) -> bool:
    ...
    **kwargs : dict
    ...
    return all(
        isinstance(
            v,
            tuple) and len(v) == 2 for v in kwargs.values()) and (
        len(kwargs) > 0)
...
def relabel_result(
    ...
    for col, fun in func.items():
        ...
        if reorder_mask:
            fun = [
                com.get_callable_name(f) if not isinstance(
                    f, str) else f for f in fun]
