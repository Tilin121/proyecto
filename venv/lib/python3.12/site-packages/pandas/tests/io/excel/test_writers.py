...
class TestExcelWriter:
    ...
    def test_int_types(self, np_type, path):
        ...
        df = DataFrame(np.random.default_rng(
            2).integers(-10, 10, size=(10, 2)), dtype=np_type)
        df.to_excel(path, sheet_name="test1")
            ...
            [
                [
                    date(
                        ...
                        1999, 9, 24)], [
                    datetime(
                        1998, 5, 26, 23, 33, 4), datetime(
                        2014, 2, 28, 13, 5, 13)], ], index=[
    ...
    def test_to_excel_interval_no_labels(self, path, using_infer_string):
        ...
        df = DataFrame(np.random.default_rng(
            2).integers(-10, 10, size=(20, 1)), dtype=np.int64)
        expected = df.copy()
    ...
    def test_write_subset_columns(
        ...
        write_frame.to_excel(
            path, sheet_name="col_subset_bug", columns=[
                "A", "B"], index=to_excel_index)
