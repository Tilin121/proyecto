...
def _get_filepath_or_buffer(
    ...
    if not (
        hasattr(
            ...
            filepath_or_buffer,
            "write")):
        msg = f"Invalid file path or buffer object type: {
...
def is_potential_multi_index(
    ...
    return bool(
        len(columns) and not isinstance(
            columns,
            ABCMultiIndex) and all(
            isinstance(
                c,
                tuple) for c in columns if c not in list(index_col)))
