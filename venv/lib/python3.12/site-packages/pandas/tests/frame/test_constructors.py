...
class TestDataFrameConstructors:
    ...
    def test_constructor_dict(self):
        datetime_series = Series(
            np.arange(
                30, dtype=np.float64), index=date_range(
    ...
    def test_constructor_dict_order_insertion(self):
        datetime_series = Series(
            np.arange(
                10, dtype=np.float64), index=date_range(
                "2020-01-01", periods=10))
        datetime_series_short = datetime_series[:5]
    ...
    def test_constructor_dict_multiindex(self):
        ...
        _d = sorted(d.items())
        df = DataFrame(d)
        expected = DataFrame([x[1] for x in _d],
        ...
        _d.insert(0, ("z", d["z"]))
        expected = DataFrame([x[1] for x in _d], index=Index(
            [x[0] for x in _d], tupleize_cols=False)).T
    ...
    def test_constructor_list_of_dataclasses_with_varying_types(self):
        ...
        expected = DataFrame({"x": [0, np.nan], "y": [3, 3], "x0": [
    ...
    def test_constructor_mix_series_nonseries(self, float_frame):
        df = DataFrame({"A": float_frame["A"], "B": list(
            float_frame["B"])}, columns=["A", "B"])
    ...
    def test_constructor_with_datetimes(self, using_infer_string):
        ...
        expected = Series([np.dtype("int64")] +
                          ...
                          2 +
                          [np.dtype("M8[s]"), np.dtype("M8[us]")], index=list("ABCDE"), )
        tm.assert_series_equal(result, expected)
                             ...
                             [Categorical(list("aabbc")),
                              SparseArray([1,
                                 ...
                                 IntervalArray([Interval(0,
                                                ...
                                                Interval(1,
                                                         5)]),
                                 PeriodArray(pd.period_range(start="1/1/2017",
                                                             end="1/1/2018",
                                                             freq="M")),
                              ],
                             )
...
class TestDataFrameConstructorWithDatetimeTZ:
    ...
    @pytest.mark.skipif(not np_version_gt2,
                        reason="StringDType only available in numpy 2 and above")
    @pytest.mark.parametrize(
        "data",
        [
    ...
    def test_from_out_of_bounds_ns_datetime(
        ...
        if box is list or (frame_or_series is Series and box is dict):
            mark = pytest.mark.xfail(
                ...
                "non-nano, but DatetimeArray._from_sequence has not", strict=True, )
    ...
    def test_from_out_of_bounds_ns_timedelta(
        ...
        if box is list or (frame_or_series is Series and box is dict):
            mark = pytest.mark.xfail(
                ...
                "to non-nano, but TimedeltaArray._from_sequence has not", strict=True, )
            request.applymarker(mark)
