...
class PythonParser(ParserBase):
    ...
    def read(self,
             rows: int | None = None) -> tuple[Index | None,
    ...
    def _exclude_implicit_index(
        ...
        return {name: alldata[i + offset]
                for i, name in enumerate(names) if i < len_alldata}, names

    # legacy
    def get_chunk(self,
                  size: int | None = None) -> tuple[Index | None,
                                                    Sequence[Hashable] | MultiIndex,
                                                    Mapping[Hashable,
                                                            ArrayLike]]:
        if size is None:
            ...
            for level, hr in enumerate(header):
                try:
                    ...
                    if 0 < self.line_pos <= hr and (
                        ...
                        # header
                        joi = list(map(str,
                                        header[:-1] if have_mi_columns else header))
    ...
    def _remove_skipped_rows(
        ...
        if self.skiprows:
            return [
                row for i,
                row in enumerate(new_rows) if not self.skipfunc(
                    i + self.pos)]
        return new_rows
