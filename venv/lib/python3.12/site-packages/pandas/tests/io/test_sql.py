...
def check_iris_frame(frame: DataFrame):
    ...
    tm.assert_series_equal(row, Series(
        [5.1, 3.5, 1.4, 0.2, "Iris-setosa"], index=frame.columns, name=0))
...
def drop_view(view_name: str, conn: sqlite3.Connection |
              sqlalchemy.engine.Engine | sqlalchemy.engine.Connection, ):
    import sqlalchemy
    ...
    if isinstance(conn, sqlite3.Connection):
        conn.execute(
            f"DROP VIEW IF EXISTS {
                sql._get_valid_sqlite_name(view_name)}")
        conn.commit()
    else:
        adbc = import_optional_dependency(
            "adbc_driver_manager.dbapi", errors="ignore")
...
def test_dataframe_to_sql_arrow_dtypes_missing(conn, request, nulls_fixture):
    # GH 52046
    pytest.importorskip("pyarrow")
    df = DataFrame({"datetime": pd.array(
...
def test_api_timedelta(conn, request):
    ...
    if conn_name == "postgresql_adbc_conn":
        ...
        expected = Series([pd.DateOffset(months=0,
                                         days=0,
...
def test_to_sql_save_index(conn, request):
    ...
    conn_name = conn
    conn = request.getfixturevalue(conn)
    df = DataFrame.from_records([(1, 2.1, "line1"), (2, 1.5, "line2")], columns=[
        "A", "B", "C"], index=["A"])
...
def test_connectable_issue_example(conn, request):
    ...
    assert (DataFrame({"test_foo_data": [0, 1, 2]}).to_sql(
        name="test_foo_data", con=conn) == 3)
    main(conn)
...
def dtype_backend_expected():
    def func(storage, dtype_backend, conn_name) -> DataFrame:
        ...
        if dtype_backend == "pyarrow":
            ...
            df = DataFrame({col: ArrowExtensionArray(
                pa.array(df[col], from_pandas=True)) for col in df.columns})
