...
class ArrowExtensionArray(
            ...
            if pa.types.is_dictionary(pa_type):
                ...
                try:
                    ...
                    if pa.types.is_string(
                            pa_array.type) or pa.types.is_large_string(
                            pa_array.type):
    ...
    def factorize(
        ...
        if encoded.length() == 0:
            ...
            indices = pa_indices.to_numpy(
                zero_copy_only=False, writable=True).astype(
                np.intp, copy=False)
    ...
    def _reduce_calc(
        ...
        if keepdims:
            if isinstance(pa_result, pa.Scalar):
                ...
                result = pa.array(
                    [pa_result], type=to_pyarrow_type(
    ...
    def _replace_with_mask(
        ...
        if isinstance(
                values,
                pa.ChunkedArray) and pa.types.is_boolean(
                values.type):
    ...
    def _str_repeat(self, repeats: int | Sequence[int]):
        if not isinstance(repeats, int):
            raise NotImplementedError(
                f"repeat is not implemented when repeats is {
    ...
    def _dt_nanoseconds(self):
        return type(self)(
            pa.array(
                self._to_timedeltaarray().nanoseconds,
                from_pandas=True,
                type=pa.int32()))
    ...
    def _dt_to_pydatetime(self):
        if pa.types.is_date(self.dtype.pyarrow_dtype):
            raise ValueError(
                f"to_pydatetime cannot be called with {
                    self.dtype.pyarrow_dtype} type. " "Convert to pyarrow timestamp type.")
        data = self._pa_array.to_pylist()
        if self._dtype.pyarrow_dtype.unit == "ns":
            data = [
