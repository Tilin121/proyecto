...
class _GroupByMixin(PandasObject, SelectionMixin):
    ...
    def _gotitem(self, key, ndim, subset=None):
        ...
        try:
            if isinstance(
                    key,
                    list) and self.key not in key and self.key is not None:
                key.append(self.key)
...
class TimeGrouper(Grouper):
    ...
    def _adjust_bin_edges(
        ...
        if self.freq.name in ("BME", "ME", "W") or self.freq.name.split(
                "-")[0] in ("BQE", "BYE", "QE", "YE", "W", ):
...
def _take_new_index(obj: NDFrameT,
                    indexer: npt.NDArray[np.intp],
                    new_index: Index,
                    axis: AxisInt = 0) -> NDFrameT:
    ...
    if isinstance(obj.index, PeriodIndex):
        ...
        if isinstance(freq, BaseOffset):
            if hasattr(freq, "_period_dtype_code"):
                ...
                raise ValueError(
                    f"Invalid offset: '{
                        freq.base}' for converting time series " f"with PeriodIndex.")
